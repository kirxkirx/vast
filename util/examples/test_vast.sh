...
if [ -d ../NMW-STL__find_NovaVul24_test ];then
 ...
 remove_test31_tmp_files_if_present
 # Run the test
 echo "NMW-STL find Nova Vul 2024 test " 1>&2
 echo -n "NMW-STL find Nova Vul 2024 test: " >> vast_test_report.txt 
 #
 cp -v bad_region.lst_default bad_region.lst
 #
 if [ -f transient_report/index.html ];then
 ...
 if [ ! -f astorb_2023.dat ];then
  ...
  if [ $? -ne 0 ];then
   TEST_PASSED=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24_error_unpacking_custom_astorb_2023.dat"
  fi
 fi
 cp astorb_2023.dat astorb.dat
 if [ $? -ne 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24_error_copying_astorb_2023.dat_to_astorb.dat"
 fi
 #################################################################
 # Set STL camera bad regions file
 if [ ! -f ../STL_bad_region.lst ];then
...
$GREP_RESULT"
   ...
   if [ $? -eq 0 ];then
    ...
    GREP_RESULT=`cat vast_summary.log vast_image_details.log`
    DEBUG_OUTPUT="$DEBUG_OUTPUT
###### NMWSTLFINDNVUL240_nonzero_ref_frame_rotation_test2 ######
$GREP_RESULT"
   fi
  else
   TEST_PASSED=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL240_NO_vast_image_details_log"
  fi
  #
  #
...
$GREP_RESULT"
  ...
  if ! [[ $TEST =~ $re ]] ; then
   ...
   TEST_PASSED=0
   TEST=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL240110a_TOO_FAR_TEST_ERROR"
  else
   if [ $TEST -eq 0 ];then
    TEST_PASSED=0
    FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL240110a_TOO_FAR_$DISTANCE_ARCSEC"
   fi
  fi
  # Klotho
  ...
  if ! [[ $TEST =~ $re ]] ; then
   ...
   TEST_PASSED=0
   TEST=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24314a_TOO_FAR_TEST_ERROR"
  else
   if [ $TEST -eq 0 ];then
    TEST_PASSED=0
    FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24314a_TOO_FAR_$DISTANCE_ARCSEC"
   fi
  fi
  # CX Sge
  ...
  if ! [[ $TEST =~ $re ]] ; then
   ...
   TEST_PASSED=0
   TEST=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24414a_TOO_FAR_TEST_ERROR"
  else
   if [ $TEST -eq 0 ];then
    TEST_PASSED=0
    FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24414a_TOO_FAR_$DISTANCE_ARCSEC"
   fi
  fi
  # 
  # UX Sge
  ...
  if ! [[ $TEST =~ $re ]] ; then
   ...
   TEST_PASSED=0
   TEST=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24614a_TOO_FAR_TEST_ERROR"
  else
   if [ $TEST -eq 0 ];then
    TEST_PASSED=0
    FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24614a_TOO_FAR_$DISTANCE_ARCSEC"
   fi
  fi
  # VW Del
...
$GREP_RESULT"
  ...
  if ! [[ $TEST =~ $re ]] ; then
   ...
   TEST_PASSED=0
   TEST=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24714a_TOO_FAR_TEST_ERROR"
  else
   if [ $TEST -eq 0 ];then
    TEST_PASSED=0
  ...
  if [ $? -ne 0 ];then
   TEST_PASSED=0
   FAILED_TEST_CODES="$FAILED_TEST_CODES NMWSTLFINDNVUL24_TMP_FILE_PRESENT"
  fi
 ...
 # Make an overall conclusion for this test
 if [ $TEST_PASSED -eq 1 ];then
  echo -e "\n\033[01;34mNMW-STL find Nova Vul 2024 test \033[01;32mPASSED\033[00m ($THIS_TEST_TIME_MIN_STR)" 1>&2
  echo "PASSED ($THIS_TEST_TIME_MIN_STR)" >> vast_test_report.txt
 else
  echo -e "\n\033[01;34mNMW-STL find Nova Vul 2024 test \033[01;31mFAILED\033[00m ($THIS_TEST_TIME_MIN_STR)" 1>&2
  echo "FAILED ($THIS_TEST_TIME_MIN_STR)" >> vast_test_report.txt
 fi
else
...
for PERIOD_SEARCH_SERVER in $PERIOD_SEARCH_SERVERS ;do
 ...
 # Apache and python web server typeset webpage differently! Trying to handle both versions:
 REMOTE_FREQUENCY_CD=`WEBBROWSER=curl ./pokaz_laflerkinman.sh ../vast_test_lightcurves/out00095_edit_edit.dat 2>/dev/null | grep -e 'L&K peak 3' -e 'L&amp;K peak 3' | awk -F '=' '{print $3}'  | awk '{printf "%.4f",$1}'`
