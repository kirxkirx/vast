...
class SecondSectionLinkButtonTest(unittest.TestCase):
    ...
    def _find_section_root(self):
        drv = self.driver
        LOG.info("Locating section that contains the line '84 Klio' ...")
        klio_line = drv.find_element(
            By.XPATH, "//pre[contains(normalize-space(.), '84 Klio')]"
        )
        anchor = klio_line.find_element(By.XPATH, "./preceding::a[@name][1]")
    ...
    def _check_mpc_submission(self, section_nodes):
        ...
        self._submit_and_check(
            mpc_submit,
            MPC_PAGE_TIMEOUT,
            lambda src: "Klio" in src,
            "The MPC page does not contain 'Klio'"
        )
        LOG.info("Word 'Klio' found in MPC page.")
    ...
    def _check_vsx_submission(self, section_nodes):
        ...
        expected_text = "There were no records that matched the search criteria."
        self._submit_and_check(
            vsx_submit,
            VSX_PAGE_TIMEOUT,
            lambda src: expected_text in src,
            "VSX page does not contain the expected 'no records' message"
        )
        LOG.info("Expected 'no records' message found in VSX page.")
