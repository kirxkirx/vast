...
for VAR in `cat vast_list_of_previously_known_variables.log` ;do
 grep "$VAR" vast_lightcurve_statistics.log
done | sort -k1 > vast_lightcurve_statistics_known_variables.log
...
for INDEX in idx00_STD idx01_wSTD idx02_skew idx03_kurt idx04_I idx05_J idx06_K idx07_L idx08_Npts idx09_MAD idx10_lag1 idx11_RoMS idx12_rCh2 idx13_Isgn idx14_Vp2p idx15_Jclp idx16_Lclp idx17_Jtim idx18_Ltim idx19_N3 idx20_excr idx21_eta idx22_E_A idx23_S_B idx24_NXS idx25_IQR  idx26_A01 idx27_A02 idx28_A03 idx29_A04 idx30_A05 ;do
 ...
 if [ "$INDEX_NAME_FOR_TITLE" == "A01" ];then
  INDEX_NAME_FOR_TITLE_GNUPLOT="a_1"
 fi
 if [ "$INDEX_NAME_FOR_TITLE" == "A02" ];then
  INDEX_NAME_FOR_TITLE_GNUPLOT="a_2"
 fi
 if [ "$INDEX_NAME_FOR_TITLE" == "A03" ];then
  INDEX_NAME_FOR_TITLE_GNUPLOT="a_3"
 fi
 if [ "$INDEX_NAME_FOR_TITLE" == "A04" ];then
  INDEX_NAME_FOR_TITLE_GNUPLOT="a_4"
 fi
 if [ "$INDEX_NAME_FOR_TITLE" == "A05" ];then
  INDEX_NAME_FOR_TITLE_GNUPLOT="a_5"
 fi
...
plot \
\"vast_lightcurve_statistics.log\" u $COLUMNS_FOR_GNUPLOT  pointtype 7 pointsize 0.5 linecolor 'dark-grey' title '', \
\"vast_lightcurve_statistics_known_variables.log\" u $COLUMNS_FOR_GNUPLOT pointtype 7 pointsize 1.0 linecolor 'black' title 'confirmed variables', \
\"vast_lightcurve_statistics_expected_plus_spread.log\" u $COLUMNS_FOR_GNUPLOT w l linecolor 'red' title 'cut-off at $STRING_SIGMA_FOR_MAG_PLOT'
" | gnuplot
...
head -n1 vast_detection_efficiency.log ; grep -e idx25_IQR -e idx21_eta -e idx26_A01 vast_detection_efficiency.log
