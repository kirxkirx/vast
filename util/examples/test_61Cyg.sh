...
if [ -d /mnt/usb/61Cyg_photoplates_test_edge_segfault ];then
 cp default.sex.61_Cyg_photoplates default.sex
 ./vast -d -u --selectbestaperture --position_dependent_correction -o -r -f /mnt/usb/61Cyg_photoplates_test_edge_segfault/SCA*__-7_-7.fits
 if [ $? -ne 0 ];then
  TEST_PASSED=0
...
# Run the test
cp default.sex.61_Cyg_photoplates default.sex
./vast -u --selectbestaperture -j -o -r -f /mnt/usb/61Cyg_photoplates_test/*
...
for XY in "482.9663100 1658.7934600" "1804.7764900 768.9890100" "138.2354000 1710.2705100" "279.6370800 384.3089000" "903.0775100 795.6828000" "1655.3160400 1404.8461900" "970.5773900 2131.2663600" ;do
 ...
 if [ "$LIGHTCURVEFILE" == "none" ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  VARIABLE_NOT_DETECTED__${XY// /_}"
 fi
 grep --quiet "$LIGHTCURVEFILE" vast_autocandidates.log
 if [ $? -ne 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  VARIABLE_NOT_SELECTED__$LIGHTCURVEFILE"
 fi
 grep --quiet "$LIGHTCURVEFILE" vast_list_of_likely_constant_stars.log
 if [ $? -eq 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  VARIABLE_MISTAKEN_FOR_CONSTANT__$LIGHTCURVEFILE"
 fi
done
...
for XY in "482.9663100 1658.7934600" "1804.7764900 768.9890100" "138.2354000 1710.2705100" "279.6370800 384.3089000" "903.0775100 795.6828000" "1655.3160400 1404.8461900" "970.5773900 2131.2663600" ;do
 ...
 if [ "$LIGHTCURVEFILE" == "none" ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  CALIB_VARIABLE_NOT_DETECTED__${XY// /_}"
 fi
 grep --quiet "$LIGHTCURVEFILE" vast_autocandidates.log
 if [ $? -ne 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  CALIB_VARIABLE_NOT_SELECTED__$LIGHTCURVEFILE"
 fi
 grep --quiet "$LIGHTCURVEFILE" vast_list_of_likely_constant_stars.log
 if [ $? -eq 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  CALIB_VARIABLE_MISTAKEN_FOR_CONSTANT__$LIGHTCURVEFILE"
 fi
done
...
for XY in "384.8848000 2162.4240700" "433.1528000 1952.2907700" "1601.8258100 293.3326100" "194.6116900 2114.5200200" "1243.8588900 596.4639300" "348.6729100 2109.0227100" "205.4760000 2129.5366200" "117.2828000 1714.1951900" "274.2864100 1762.4866900" "89.8596000 730.3389900" "538.6436200 1444.2902800" "384.4455000 2162.3833000" "724.4339000 1412.2819800" "1545.0174600 2119.8383800" ;do
 ...
 if [ $? -eq 0 ];then
  TEST_PASSED=0
  FAILED_TEST_CODES="$FAILED_TEST_CODES  CALIB_FALSE_CANDIDATE_SELECTED__$LIGHTCURVEFILE"
 fi
done
