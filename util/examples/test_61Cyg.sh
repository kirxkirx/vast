...
if [ ! -d "$TEST_DIR" ];then
 ...
 if [ ! -d "$TEST_DIR" ];then
  ...
  #######################
  curl -L --fail -O http://scan.sai.msu.ru/~kirx/data/vast_tests/61Cyg_photoplates_test_edge_segfault.tar.bz2 && tar -xf 61Cyg_photoplates_test_edge_segfault.tar.bz2 && rm -f 61Cyg_photoplates_test_edge_segfault.tar.bz2
...
if [ ! -d "$TEST_DIR" ];then
 ...
 if [ ! -d "$TEST_DIR" ];then
  cd ..
  curl -L --fail -O http://scan.sai.msu.ru/~kirx/data/vast_tests/61Cyg_photoplates_test.tar.bz2 && tar -xf 61Cyg_photoplates_test.tar.bz2 && rm -f 61Cyg_photoplates_test.tar.bz2
  WGET_AND_UNPACK_RETURN_STATUS=$?
  cd -
  if [ $WGET_AND_UNPACK_RETURN_STATUS -ne 0 ];then
   echo "ERROR: test data not found: $TEST_DIR"
