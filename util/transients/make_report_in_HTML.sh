...
# Finding-chart thumbnails (reference/discovery) are sharp, few-color images that
# compress ~40% smaller as LOSSLESS WebP with no quality loss. Full-frame previews
# are photographic and do NOT benefit from lossless WebP, so they stay PNG.
# Pick a lossless WebP encoder once: cwebp, else ImageMagick (which MUST be forced
# to lossless -- its default is lossy and larger than the PNG), else keep PNG.
# THUMB_EXT is used for the thumbnail <img> tags and the conversion pass near the
# end of this script. Each machine decides independently, so a box with no WebP
# encoder simply keeps PNG.
THUMB_EXT="png"
WEBP_ENCODER="none"
if command -v cwebp >/dev/null 2>&1; then
 THUMB_EXT="webp"
 WEBP_ENCODER="cwebp"
elif command -v convert >/dev/null 2>&1 && convert -list format 2>/dev/null | grep -i webp | grep -q 'rw'; then
 THUMB_EXT="webp"
 WEBP_ENCODER="imagemagick"
fi

# Convert one finding-chart PNG to lossless WebP in place (writes <base>.webp and
# removes the PNG on success). Leaves the PNG untouched if conversion fails.
function convert_thumbnail_png_to_webp() {
 local png_file="$1"
 local webp_file="${png_file%.png}.webp"
 case "$WEBP_ENCODER" in
  cwebp)
   cwebp -quiet -lossless "$png_file" -o "$webp_file" 2>/dev/null && rm -f "$png_file" ;;
  imagemagick)
   convert "$png_file" -define webp:lossless=true "$webp_file" 2>/dev/null && rm -f "$png_file" ;;
  *)
   return 1 ;;
 esac
}
...
<script>printCandidateNameWithAbsLink('$TRANSIENT_NAME');</script>" >> transient_report/index.tmp
 ...
 #####
 echo "<img src=\""$TRANSIENT_NAME"_reference.$THUMB_EXT\">" >> transient_report/index.tmp
 ...
 while read JD MAG ERR X Y APP IMAGE REST ;do
  ...
  if [ "$IMAGE" != "$REFERENCE_IMAGE" ];then
   ...
   #####
   echo "<img src=\""$TRANSIENT_NAME"_discovery"$N".$THUMB_EXT\">" >> transient_report/index.tmp
...
# Convert finding-chart thumbnails (reference/discovery) to lossless WebP; the
# full-frame previews stay PNG. All thumbnails are fully generated by now, so a
# single pass here leaves the per-candidate generation/fallback logic untouched.
# The <img> tags above already reference the .$THUMB_EXT extension.
if [ "$THUMB_EXT" = "webp" ];then
 wait
 for THUMBNAIL_PNG in transient_report/*_reference.png transient_report/*_discovery*.png ;do
  [ -f "$THUMBNAIL_PNG" ] || continue
  convert_thumbnail_png_to_webp "$THUMBNAIL_PNG"
 done
fi
