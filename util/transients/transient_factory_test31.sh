...
# Disable flag image check to speed-up processing
VAST_OPTION_NOFLAGIMAGE="--noflagimage"
...
#  NUMBER_OF_DETECTED_TRANSIENTS_BEFORE_FILTERING_HARD_LIMIT=3000
  NUMBER_OF_DETECTED_TRANSIENTS_BEFORE_FILTERING_SOFT_LIMIT=8000
  NUMBER_OF_DETECTED_TRANSIENTS_BEFORE_FILTERING_HARD_LIMIT=10000
...
#  NUMBER_OF_DETECTED_TRANSIENTS_BEFORE_FILTERING_HARD_LIMIT=1500
 ...
 #
 #
 if [ "$CAMERA_SETTINGS" = "TTUQ1b1x1" ];then
  ...
  if [ -z "$FLAT_FIELD_DIR_OR_FILE" ];then
   ...
   export FLAT_FIELD_DIR_OR_FILE="$NMW_CALIBRATION/$CAMERA_SETTINGS/flats"
  fi
  #
  #TELESCOP_NAME_KNOWN_TO_VaST_FOR_FOV_DETERMINATION="STL-11000M"
  #export TELESCOP_NAME_KNOWN_TO_VaST_FOR_FOV_DETERMINATION
...
#  NUMBER_OF_DETECTED_TRANSIENTS_BEFORE_FILTERING_HARD_LIMIT=8000
  ...
  # Nope, looks like we can avoid many mismatches with a larger edge ident
  #FRAME_EDGE_OFFSET_PIX=100
  # Trying to mask bad focus regions
  FRAME_EDGE_OFFSET_PIX=600
  # Do not compress fully calibrated images to reduce disk load
  FPACK_FULLY_CALIBRATED_IMAGE="no"
 fi
 #
...
# given image basename in transient_factory_test31.txt. Echoes the value, or
# empty string if the field is absent or NaN.
function extract_wcs_quality_field {
 awk -v img="$1" -v fld="$2" '
...
function average_two_numbers {
 if [ -n "$1" ] && [ -n "$2" ]; then
  awk -v a="$1" -v b="$2" 'BEGIN { print (a + b) / 2.0 }'
 elif [ -n "$1" ]; then
  echo "$1"
 elif [ -n "$2" ]; then
  echo "$2"
...
# Compare a new-image WCS quality metric to the reference-image average; emit
# WARNING (which the _summary page picks up via the existing log machinery) if
# the new value exceeds the reference value by more than the configured ratio.
# Args: label new_value ref_value metric_name
function warn_if_wcs_quality_worse_than_reference {
 local label="$1" new_val="$2" ref_val="$3" metric="$4"
 local ratio
 if [ -z "$new_val" ] || [ -z "$ref_val" ]; then
  echo "INFO: WCS quality $metric not available for comparison ($label new=${new_val:-N/A} ref_avg=${ref_val:-N/A})" | tee -a transient_factory_test31.txt
  return
 fi
 if awk -v n="$new_val" -v r="$ref_val" -v t="$WCS_QUALITY_RATIO_THRESHOLD" \
        'BEGIN { exit !(r > 0 && (n / r) > t) }' ; then
  ratio=$(awk -v n="$new_val" -v r="$ref_val" 'BEGIN { printf "%.2f", n / r }')
  echo "WARNING: $label has $metric=$new_val -- ${ratio}x the reference average ($ref_val); plate solution may be of low accuracy" | tee -a transient_factory_test31.txt
 fi
}
...
util/solve_plate_with_UCAC5 --iterations $UCAC5_PLATESOLVE_ITERATIONS $REFERENCE_EPOCH__FIRST_IMAGE" | tee -a transient_factory_test31.txt
  ...
  record_timing "      FILTER_FRAME_EDGE" "$FILTER_START_UNIXSEC"
  ############################################
