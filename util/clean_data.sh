...
for i in candidates-*.err candidates-*.txt candidates-*.lst m_sigma_bin.tmp match.txt calib.txt calibration_catalog_stars_in_frame.count calib.txt_param calib.txt_param_tycho2 calib.png manually_selected_comparison_stars.lst manually_selected_aperture.txt ;do
 if [ -f "$i" ];then
  rm -f "$i"
 fi
done
...
for i in vast*.log ucac5_server_status.log gaia_dr2_test_output.log vast_list_of_all_stars.ds9 vast_list_of_all_stars.ds9.reg wcs.fit m_sigma_bin.tmp sysrem_input_star_list.lst* ref_frame_sextractor.cat util/convert/*~ bright_star_blend_check_*.sex ;do
 if [ -f "$i" ];then
  rm -f "$i"
 fi
done
