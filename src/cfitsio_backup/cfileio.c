...
*/
{
 ...
 while ( *tstr ) {
   ...
   *string= tstr + 1;   /* set pointer to next char */
   ...
   tstr++;
   if ( find_bracket( &tstr ) )
    return ( 1 );
  } else if ( *tstr == '{' ) {
   tstr++;
   if ( find_curlybracket( &tstr ) )
    return ( 1 );
  } else if ( *tstr == '"' ) {
   tstr++;
   if ( find_doublequote( &tstr ) )
    return ( 1 );
  } else if ( *tstr == '\'' ) {
   tstr++;
   if ( find_quote( &tstr ) )
...
*/
{
 ...
 if ( *cptr == '@' ) {
  if ( ffimport_file( cptr + 1, &file_expr, status ) )
   return ( *status );
  cptr= file_expr;
  while ( *cptr == ' ' )
 ...
 while ( ( slen= fits_get_token2( &cptr, ";", &clause, NULL, status ) ) > 0 ) {
  ...
  if ( clause )
   free( clause ); /* free old clause before getting new one */
  clause= NULL;
 }
...
*/
{
 ...
 if ( *ptr1 == '-' && ( *( ptr1 + 1 ) == 0 || *( ptr1 + 1 ) == ' ' ||
  ...
  if ( ptr2 ) /* copy the explicit urltype string */
  {
   ...
   ptr1= ptr2 + 3;
  } else if ( !strncmp( ptr1, "ftp:", 4 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "ftp://" );
   ptr1+= 4;
  } else if ( !strncmp( ptr1, "gsiftp:", 7 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "gsiftp://" );
   ptr1+= 7;
  } else if ( !strncmp( ptr1, "http:", 5 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "http://" );
   ptr1+= 5;
  } else if ( !strncmp( ptr1, "mem:", 4 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "mem://" );
   ptr1+= 4;
  } else if ( !strncmp( ptr1, "shmem:", 6 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "shmem://" );
   ptr1+= 6;
  } else if ( !strncmp( ptr1, "file:", 5 ) ) { /* the 2 //'s are optional */
   if ( urltype )
    strcat( urltype, "file://" );
   ptr1+= 5;
...
*/
 ...
 if ( ptr1 ) { /* find the end of the column specifier */
  ptr2= ptr1 + 5;
  while ( *ptr2 != ']' ) {
   if ( *ptr2 == '\0' ) {
    ffpmsg( "input file URL is missing closing bracket ']'" );
   ...
   if ( *ptr2 == '\'' ) /* start of a literal string */
   {
    ...
    if ( !ptr2 ) {
     ffpmsg( "literal string in input file URL is missing closing single quote" );
     free( infile );
     return ( *status= URL_PARSE_ERROR ); /* error, no closing ] */
    }
   }
   ...
   if ( *ptr2 == '[' ) /* set of nested square brackets */
   {
    ...
    if ( !ptr2 ) {
     ffpmsg( "nested brackets in input file URL is missing closing bracket" );
     free( infile );
     return ( *status= URL_PARSE_ERROR ); /* error, no closing ] */
    }
   }
 ...
 if ( ptr1 ) { /* find the end of the pixel filter */
  while ( *ptr2 != ']' ) {
   if ( *ptr2 == '\0' ) {
    ffpmsg( "input file URL is missing closing bracket ']'" );
    free( infile );
...

{
 ...
 if ( ( ( *ptr1 == '-' ) && ( *( ptr1 + 1 ) == 0 || *( ptr1 + 1 ) == ' ' ) ) || !strcmp( ptr1, "stdout" ) || !strcmp( ptr1, "STDOUT" ) )
 ...
 /* e.g., "-55d33m.fits"   */
 {
  ...
  if ( urltype && outfile ) {
   if ( !strcmp( urltype, "file://" ) ) {
    ...
    if ( ptr1 ) { /* make sure the ".gz" is at the end of the file name */
     ptr1+= 3;
     if ( *ptr1 == 0 || *ptr1 == ' ' )
...
*/
 ...
 if ( *ptr == '\0' )
  return ( 0 ); /* no remaining file names */
...
int pixel_filter_helper(
 ...
 case 'b':
 case 'B':
  filter.bitpix= BYTE_IMG;
  break;
 case 'i':
 case 'I':
  filter.bitpix= SHORT_IMG;
  break;
 case 'j':
 case 'J':
  filter.bitpix= LONG_IMG;
  break;
 case 'r':
 case 'R':
  filter.bitpix= FLOAT_IMG;
  break;
 case 'd':
 case 'D':
  filter.bitpix= DOUBLE_IMG;
  break;
 }
