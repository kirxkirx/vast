   ...
   *status= fits_read_key_lng( tmpfptr, keyword, &grpid, card, status );
   ...
   if ( grpid == groupExtver ) {
     ...
     *status= fits_read_key_longstr( mfptr, keyword, &tgrplc, card,
     ...
     if ( strcmp( grplc, groupLocation ) == 0 )
      found= 1;
    } else {
...

{
  ...
  *status= ffgtgc( gfptr, &xtensionCol, &extnameCol, &extverCol, &positionCol,
   ...
   *status= fits_read_col_str( gfptr, xtensionCol, member, 1, 1, nstr,
   ...
   if ( fits_strcasecmp( xtension, "PRIMARY" ) == 0 )
    hdutype= IMAGE_HDU;
   else if ( fits_strcasecmp( xtension, "IMAGE" ) == 0 )
    hdutype= IMAGE_HDU;
   else if ( fits_strcasecmp( xtension, "TABLE" ) == 0 )
    hdutype= ASCII_TBL;
   else if ( fits_strcasecmp( xtension, "BINTABLE" ) == 0 )
    hdutype= BINARY_TBL;
   else
    hdutype= ANY_HDU;
  }
 ...
 *status= fits_get_num_members( gfptr, &nmembers, status );
 ...
 for ( i= 1; i <= nmembers && *member == 0 && *status == 0; ++i ) {
  if ( xtensionCol != 0 ) {
   ...
   if ( fits_strcasecmp( tmpPtr[0], xtension ) != 0 )
    continue;
  }

  if ( extnameCol != 0 ) {
   fits_read_col_str( gfptr, extnameCol, i, 1, 1, nstr, tmpPtr, &dummy, status );
   if ( fits_strcasecmp( tmpPtr[0], extname ) != 0 )
    continue;
  }
  ...
  if ( strcmp( mbrLocation1, tmpLocation ) != 0 &&
       strcmp( mbrLocation2, tmpLocation ) != 0 )
   continue;
...

{
 ...
 do {
  ...
  for ( i= 0;
        i < HDU->nHDU && !( HDU->position[i] == hdunum && strcmp( HDU->filename[i], filename2 ) == 0 );
        ++i )
   ;
  ...
  if ( i != HDU->nHDU ) {
   ...
   if ( newPosition != NULL )
    *newPosition= HDU->newPosition[i];
   if ( newFileName != NULL )
    strcpy( newFileName, HDU->newFilename[i] );
   continue;
  }
 ...
 do {
  ...
  while ( tmp ) {
   if ( !strcmp( tmp, ".." ) ) {
    ...
    if ( 0 < mystack->stack_size )
     pop_grp_stack( mystack );
    else if ( '/' != *inURL )
     push_grp_stack( mystack, tmp );
   } else {
...

{
 ...
 while ( *p != 0 ) {
  ...
  if ( *p == HEX_ESCAPE ) {
   if ( ( c= *( ++p ) ) != 0 ) {
    *q= ( ( c >= '0' && c <= '9' ) ? ( c - '0' ) : ( ( c >= 'A' && c <= 'F' ) ? ( c - 'A' + 10 ) : ( c - 'a' + 10 ) ) ) * 16;

    if ( ( c= *( ++p ) ) != 0 ) {
     *q= *q + ( ( c >= '0' && c <= '9' ) ? ( c - '0' ) : ( ( c >= 'A' && c <= 'F' ) ? ( c - 'A' + 10 ) : ( c - 'a' + 10 ) ) );
     p++, q++;
    }
   }
  } else
