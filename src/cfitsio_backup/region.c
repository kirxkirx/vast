   ...
   while ( *currLoc != '\0' ) {
    ...
    while ( !done && !*status && *currLoc ) {
     ...
     case ':':
      currLoc++;
      if ( paramPtr )
       cFmt= hhmmss_fmt; /* set format if parameter has : */
      break;
     case 'd':
      currLoc++;
      if ( paramPtr )
       cFmt= degree_fmt; /* set format if parameter has d */
      break;
     case ',':
    ...
    if ( !( aRgn->nShapes % 10 ) ) {
     if ( aRgn->Shapes )
      tmpShape= (RgnShape *)realloc( aRgn->Shapes,
                                     ( 10 + aRgn->nShapes ) * sizeof( RgnShape ) );
     else
    ...
    if ( !fits_strcasecmp( namePtr, "circle" ) ) {
     ...
     newShape->param.gen.p[7]= 0.0;
     nCoords= 2;
    } else if ( !fits_strcasecmp( namePtr, "box" ) || !fits_strcasecmp( namePtr, "rotbox" ) ) {
     if ( nParams < 4 || nParams > 8 ) {
      *status= PARSE_SYNTAX_ERR;
     } else if ( nParams < 6 ) {
     ...
     nCoords= 2;
    } else if ( !fits_strcasecmp( namePtr, "rectangle" ) || !fits_strcasecmp( namePtr, "rotrectangle" ) ) {
     newShape->shape= rectangle_rgn;
     if ( nParams < 4 || nParams > 5 )
      *status= PARSE_SYNTAX_ERR;
     newShape->param.gen.p[4]= 0.0;
     nCoords= 4;
    } else if ( !fits_strcasecmp( namePtr, "diamond" ) || !fits_strcasecmp( namePtr, "rotdiamond" ) || !fits_strcasecmp( namePtr, "rhombus" ) || !fits_strcasecmp( namePtr, "rotrhombus" ) ) {
     newShape->shape= diamond_rgn;
     if ( nParams < 4 || nParams > 5 )
      *status= PARSE_SYNTAX_ERR;
     newShape->param.gen.p[4]= 0.0;
     nCoords= 2;
    } else if ( !fits_strcasecmp( namePtr, "sector" ) || !fits_strcasecmp( namePtr, "pie" ) ) {
    ...
    if ( newShape->shape == poly_rgn ) {
     newShape->param.poly.Pts= (double *)malloc( nParams * sizeof( double ) );
 ...
 for ( i= 0; i < Rgn->nShapes; i++, Shapes++ ) {
  ...
  if ( ( !comp_result && Shapes->sign ) || ( comp_result && !Shapes->sign ) ) {
   ...
   case line_rgn:
    ...
    if ( ( y < -0.5 ) || ( y >= 0.5 ) || ( x < -0.5 ) || ( x >= Shapes->param.gen.a ) )
     comp_result= 0;
    break;
  ...
  if ( !aRgn->Shapes[i].sign ) {
   ...
   while ( j >= 0 ) {
    ...
    if ( aRgn->Shapes[j].sign ) {
     ...
     aRgn->nShapes++;
     for ( k= aRgn->nShapes - 1; k > j + 1; k-- )
      aRgn->Shapes[k]= aRgn->Shapes[k - 1];
...
void fits_setup_shape( RgnShape *newShape ) {
 ...
 case rectangle_rgn:
  ...
  newShape->param.gen.a= fabs( X * newShape->param.gen.cosT + Y * newShape->param.gen.sinT );
  newShape->param.gen.b= fabs( Y * newShape->param.gen.cosT - X * newShape->param.gen.sinT );
...
/*---------------------------------------------------------------------------*/
{
 ...
 for ( i= 1; i <= aRgn->nShapes; i++ ) {
  ...
  for ( j= 0; j < 9; j++ ) {
   if ( !strcmp( cvalue2, shapename[j] ) )
    newShape->shape= shapetype[j];
  }
  ...
  npos= 1;
  if ( newShape->shape == poly_rgn )
   npos= newShape->param.poly.nPts / 2;
  if ( newShape->shape == rectangle_rgn )
   npos= 2;
