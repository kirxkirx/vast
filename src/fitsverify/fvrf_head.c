...
int verify_fits( char *infile, FILE *out ) {
 ...
 pfile= p;
 p+= ( len - 1 );
 for ( i= len - 1; i >= 0 && isspace( (int)*p ); i-- ) {
  *p= '\0';
  p--;
 }
 if ( !strlen( pfile ) )
...
void init_hdu( fitsfile *infits, /* input fits file   */
 ...
 for ( j= 3; j < 3 + hduptr->naxis; j++ ) {
  ...
  if ( !isdigit( (int)*p ) )
   continue;
#if ( USE_LL_SUFFIX == 1 )
  if ( check_int( &tmpkey, out ) )
   lu= strtoll( tmpkey.kvalue, NULL, 10 );
#else
  if ( check_int( &tmpkey, out ) )
   lu= strtol( tmpkey.kvalue, NULL, 10 );
#endif
  lv= strtol( p, NULL, 10 );
 ...
 for ( i= 0; i < numusrkey; i++ )
  tmpkwds[i]= kwds[i]->kname;
...

{
 ...
 if ( k >= 0 ) {
  ...
  if ( check_int( kwds[j], out ) ) {
   ...
   nmax= wcsaxesvalue;
   if ( wcsaxesvalue > wcsaxes )
    wcsaxes= wcsaxesvalue;
   wcsaxesExists= 1;

   /* store index of the wcsaxes keyword */
   /*  (it must appear before other WCS keywords) */
   if ( pkey->kindex > wcsaxespos )
    wcsaxespos= pkey->kindex;
  }
 }
...
*/
  ...
  p= pkey->kvalue;
  while ( *p != ' ' && *p != '\0' ) {
   if ( !isdigit( (int)*p ) && !isupper( (int)*p ) && *p != '.' && *p != ')' && *p != '(' ) {
    sprintf( errmes,
...
void test_bin_ext( fitsfile *infits, /* input fits file   */
 ...
 for ( j= k; j < k + n; j++ ) {
  ...
  if ( strchr( tform[i], 'B' ) != NULL && ( l < 0 || l > 255 ) ) {
  ...
  l= strtol( kwds[j]->kvalue, NULL, 10 );
  if ( strchr( tform[i], 'I' ) != NULL && ( l < -32768 || l > 32767 ) ) {
   sprintf( errmes, "Keyword #%d, %s: The value %ld",
            kwds[j]->kindex, kwds[j]->kname, l );
...
void print_title( FILE *out, int hdunum, int hdutype ) {
 ...
 oldhdu= curhdu;
 if ( curhdu == totalhdu )
  oldhdu= 0; /* reset the old hdu at the last hdu */
 return;
}
