...
int wrtwrn( FILE *out, char *mess, int isheasarc ) {
 if ( err_report )
  return 0; /* Don't print the warnings */
 if ( !heasarc_conv && isheasarc )
  return 0; /* heasarc warnings  but with
...
/* dump the cfitsio stack */
{
 ...
 if ( out != NULL ) {
  if ( ( out != stdout ) && ( out != stderr ) ) {
   print_fmt( out, temp, 13 );
   for ( i= 0; i <= nstack; i++ )
    fprintf( out, errfmt, tmp[i] );
  }

#ifdef ERR2OUT
  print_fmt( stdout, temp, 13 );
  for ( i= 0; i <= nstack; i++ )
   fprintf( stdout, errfmt, tmp[i] );
#else
  print_fmt( stderr, temp, 13 );
  for ( i= 0; i <= nstack; i++ )
   fprintf( stderr, errfmt, tmp[i] );
#endif
 }
    ...
    Continue lines are aligned. */
{
 ...
 if ( i <= 0 ) {
  ...
  while ( *p != '\0' && i > 0 ) {
   ...
   tmp[clen]= '\0';
   i= strlen( p ) - clen;
   if ( i > 0 && isprint( (int)*( p + clen - 1 ) ) && isprint( (int)*( p + clen ) ) && *( p + clen ) != '\0' ) {
    j= clen;
    while ( *( p + j ) != ' ' && j > 0 )
     j--;
    p+= j;
...
/* print a line of char fill with string title in the middle */
{
  ...
  *p= '\0';
  strcat( line, title );
  p+= ntitle;
  for ( i= first_end + ntitle; i < nchar; i++ ) {
   *p= fill;
   p++;
  }
  *p= '\0';
