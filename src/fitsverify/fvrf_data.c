...

{
 ...
 if ( hduptr->hdutype == ASCII_TBL ) {
  ...
  for ( i= 0; i < ncols; i++ ) {
   ...
   } else if ( datatype == TBIT && ( repeat % 8 ) ) { /* bit column that does not have a multiple of 8 bits */
    numlist[nnum]= i + 1;
    nnum++;
 ...
 if ( niter )
  iter_col= (iteratorCol *)malloc( sizeof( iteratorCol ) * niter );
 ...
 for ( jl= 1; jl <= totalrows; jl++ ) {
  for ( i= 0; i < ndesc; i++ ) {
   ...
   if ( !length )
    continue; /* skip the 0 length array */
