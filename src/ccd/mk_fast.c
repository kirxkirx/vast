...
// as it tends to hit O(n^2) worst-case performance. So here we use histogram-based median for each image.
// Then for each pixel median we use quickselect-based median and it wors just fine (faster than full sort).
//
...
// Median of unsigned short data via 16-bit histogram
static double median_from_histogram_u16( const unsigned int *hist, long n ) {
 long target1= ( n - 1 ) / 2;
 long target2= n / 2;
 long cum= 0;
 int v1= -1;
...
int main( int argc, char *argv[] ) {
 ...
 unsigned int center_hist[65536];
 ...
 */
 ...
 for ( file_counter= 0; file_counter < loaded_file_counter; file_counter++ ) {
  ...
  if ( naxes_ref[0] >= CENTER_BOX_SIZE && naxes_ref[1] >= CENTER_BOX_SIZE ) {
   ...
   memset( center_hist, 0, 65536 * sizeof( unsigned int ) );
   center_box_count= 0;
   for ( by= y0; by < y1; by++ ) {
    for ( bx= x0; bx < x1; bx++ ) {
     center_hist[image_array[file_counter][by * naxes_ref[0] + bx]]++;
     center_box_count++;
    }
   }
   center_median= median_from_histogram_u16( center_hist, center_box_count );
