...
static void compute_noise_model(const float *median5_image, float *noise,
    ...
    for (i = 0; i < npix; i++) {
        ...
        if (med_val < 1.0e-5f) med_val = 1.0e-5f;
...
static int lacosmic_process(float *image, unsigned char *crmask, long width, long height,
    ...
    if (!upsampled || !laplacian || !med5_img || !noise || !snr ||
        ...
        free(upsampled); free(laplacian); free(med5_img); free(noise);
        free(snr); free(snr_medsub); free(med3_img); free(med7_img);
        free(fine_struct); free(iter_mask); free(dilated); free(work_buffer);
        return -1;
    }
    ...
    *header_keys = malloc(*num_keys * sizeof(char *));
    ...
    for (ii = 0; ii < *num_keys; ii++) {
        ...
        if ((*header_keys)[ii] == NULL) {
            ...
            for (j = 0; j < ii; j++) free((*header_keys)[j]);
            free(*header_keys);
            free(*data);
            fits_close_file(fptr, &status);
            return -1;
...
static int is_structural_keyword(const char *record) {
    if (strncmp(record, "SIMPLE  ", 8) == 0) return 1;
    if (strncmp(record, "BITPIX  ", 8) == 0) return 1;
    if (strncmp(record, "NAXIS   ", 8) == 0) return 1;
    if (strncmp(record, "NAXIS1  ", 8) == 0) return 1;
    if (strncmp(record, "NAXIS2  ", 8) == 0) return 1;
    if (strncmp(record, "EXTEND  ", 8) == 0) return 1;
    if (strncmp(record, "BZERO   ", 8) == 0) return 1;
    if (strncmp(record, "BSCALE  ", 8) == 0) return 1;
    if (strncmp(record, "END", 3) == 0 && (record[3] == ' ' || record[3] == '\0')) return 1;
    return 0;
}
        ...
        case SHORT_IMG:
            /* For 16-bit unsigned output, we need to set BZERO=32768 and convert manually */
            {
                ...
                for (i = 0; i < npixels; i++) {
                    ...
                    if (val < 1.0f) val = 1.0f;  /* Avoid 0 to not confuse VaST flag image */
                    if (val > 65534.0f) val = 65534.0f;
...
int main(int argc, char *argv[]) {
    ...
    if (crmask == NULL) {
        ...
        free(image_data);
        for (ii = 0; ii < num_keys; ii++) free(header_keys[ii]);
        free(header_keys);
        return EXIT_FAILURE;
    }
