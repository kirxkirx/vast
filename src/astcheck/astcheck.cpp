...
FILE *get_mpcorb_file( void)
{
   ...
   if( ifile)
      ...
      if( i > 70)       /* put date into YYYYMMDD form */
         {
         double jd = get_time_from_string( 0., buff + 60, 0);
...
int main( int argc, char **argv)
{
   ...
   double tolerance_in_arcsec = 900.;       /* = 15' = 1/4 degree */
   double mag_limit = 21.;
   ...
   if(ilines == NULL){
        fprintf(stderr, "ERROR: Couldn't allocate memory for ilines\n");
        exit(1);
    };
   n_ilines = 0;
   fseek( ifile, 0L, SEEK_SET);
   while( fgets( buff, sizeof( buff), ifile))
      if( !get_mpc_data( buff, &jd, &ra, &dec))
         ...
         if(ilines[n_ilines] == NULL){
            fprintf(stderr, "ERROR: Couldn't allocate memory for ilines[n_ilines]\n");
            exit(1);
        };
...
//                      sprintf( tbuff + strlen( tbuff), "  %d\"", ephem_uncertainties[i]);
                        for( j = 0; j < n_results
                        ...
                        if(results[j] == NULL){
                            fprintf(stderr, "ERROR: Couldn't allocate memory for results[j]\n");
                            exit(1);
                        };
