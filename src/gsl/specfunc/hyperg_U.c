...
hyperg_U_CF1( const double a, const double b, const int N, const double x,
 ...
 while ( n < maxiter ) {
  ...
  if ( fabs( del - 1.0 ) < 10.0 * GSL_DBL_EPSILON )
   break;
 }
 ...
 if ( N <= 0 ) {
  ...
  if ( M < 0 ) {
   ...
   if ( stat_gbm1 == GSL_SUCCESS ) {
    ...
    double coeff_err= gbm1.err * fabs( gamr.val * pe_val ) + gamr.err * fabs( gbm1.val * pe_val ) + fabs( gbm1.val * gamr.val ) * pe_err + 2.0 * GSL_DBL_EPSILON * fabs( coeff_val );
...
hyperg_U_infinite_sum_stable( int N, double a, double bint, double b, double beps, double x, double xeps, gsl_sf_result sum,
 ...
 double factor_err= fabs( gamr.val ) * powx.err + fabs( powx.val ) * gamr.err + 2.0 * GSL_DBL_EPSILON * fabs( factor_val );
 ...
 double b0_err= fabs( factor_val * pochaxibeps.val * gamrni.val ) * gamrxi1beps.err + fabs( factor_val * pochaxibeps.val * gamrxi1beps.val ) * gamrni.err + fabs( factor_val * gamrni.val * gamrxi1beps.val ) * pochaxibeps.err + fabs( pochaxibeps.val * gamrni.val * gamrxi1beps.val ) * factor_err + 2.0 * GSL_DBL_EPSILON * fabs( b0_val );
 ...
 double a0_err= fabs( factor_val * pochai.val * dgamrbxi.val / beps ) * gamri1.err + fabs( factor_val * pochai.val * gamri1.val / beps ) * dgamrbxi.err + fabs( factor_val * dgamrbxi.val * gamri1.val / beps ) * pochai.err + fabs( pochai.val * dgamrbxi.val * gamri1.val / beps ) * factor_err + 2.0 * GSL_DBL_EPSILON * fabs( a0_val );
 ...
 dchu_err= sum.err + a0_err + b0_err + 2.0 * GSL_DBL_EPSILON * ( fabs( sum.val ) + fabs( a0_val ) + fabs( b0_val ) );
...
hyperg_U_infinite_sum_simple( int N, double a, double bint, double b, double beps, double x, double xeps, gsl_sf_result sum,
 ...
 for ( i= 1; i < 2000; i++ ) {
  ...
  dchu_val+= t_val;
  dchu_err+= t_err;
  if ( !gsl_finite( t_val ) || fabs( t_val ) < EPS * fabs( dchu_val ) )
   break;
 }
...
hyperg_U_infinite_sum_improved( int N, double a, double bint, double b, double beps, double x, double xeps, gsl_sf_result sum,
 ...
 double xeps1_err= 2.0 * GSL_DBL_EPSILON * ( 1.0 + fabs( beps * lnx ) ) * fabs( dexprl.val ) + fabs( lnx ) * dexprl.err + 2.0 * GSL_DBL_EPSILON * fabs( xeps1_val );
 double dchu_val= sum.val + c0_val + xeps1_val * b0_val;
 double dchu_err= sum.err + c0_err + fabs( xeps1_val ) * b0_err + xeps1_err * fabs( b0_val ) + fabs( b0_val * lnx ) * dexprl.err + 2.0 * GSL_DBL_EPSILON * ( fabs( sum.val ) + fabs( c0_val ) + fabs( xeps1_val * b0_val ) );
 double xn= N;
 double t_val;
 double t_err;
 ...
 for ( i= 1; i < 2000; i++ ) {
  ...
  c0_err= fabs( c0_multiplier_1 ) * c0_err + fabs( c0_multiplier_2 ) * b0_err + fabs( c0_val ) * 8.0 * 2.0 * GSL_DBL_EPSILON + fabs( b0_val * c0_multiplier_2 ) * 16.0 * 2.0 * GSL_DBL_EPSILON;
...
hyperg_U_series( const double a, const double b, const double x, gsl_sf_result *result ) {
 ...
 if ( a_beps_int && a_beps <= 0 ) {
  beps= beps - 1 + floor( a_beps );
  bint= bint + 1 - floor( a_beps );
 }
...
hyperg_U_small_a_bgt0( const double a, const double b, const double x,
  ...
  *ln_multiplier= 0.0;
  return GSL_SUCCESS;
 } else if ( ( b > 5000.0 && x < 0.90 * fabs( b ) ) || ( b > 500.0 && x < 0.50 * fabs( b ) ) ) {
 ...
 if ( a == 0 ) {
  ...
  if ( b <= x ) {
   ...
   if ( Ua1_bck_val == 0.0 ) {
    ...
    double lnr_err= lm_for.err + ln_for_err + ln_bck_err + 2.0 * GSL_DBL_EPSILON * ( fabs( lm_for.val ) + fabs( ln_for_val ) + fabs( ln_bck_val ) + fabs( lns ) );
...
hyperg_U_negx( const double a, const double b, const double x, gsl_sf_result_e10 *result ) {
 ...
 if ( b_int && b <= 0 && !( a_int && a <= 0 && a >= b ) ) {
  ...
  if ( r1.val != 0.0 ) {
   ...
   T1= Mr1.val * r1.val;
   T1_err= 2.0 * GSL_DBL_EPSILON * fabs( T1 ) + fabs( Mr1.err * r1.val ) + fabs( Mr1.val * r1.err );
  }
 }
 ...
 if ( b_int && b >= 2 && !( a_int && a <= ( b - 2 ) ) ) {
  ...
  if ( r2.val != 0.0 ) {
   ...
   T2= Mr2.val * r2.val;
   T2_err= 2.0 * GSL_DBL_EPSILON * fabs( T2 ) + fabs( Mr2.err * r2.val ) + fabs( Mr2.val * r2.err );
...
int gsl_sf_hyperg_U_int_e10_e( const int a, const int b, const double x,
                               gsl_sf_result_e10 *result ) {
...
int gsl_sf_hyperg_U_e10_e( const double a, const double b, const double x,
                           gsl_sf_result_e10 *result ) {
 const double rinta= floor( a + 0.5 );
...
int gsl_sf_hyperg_U_int_e( const int a, const int b, const double x, gsl_sf_result *result ) {
