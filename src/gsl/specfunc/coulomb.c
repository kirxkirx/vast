...
int gsl_sf_coulomb_CL_e( double lam, double eta, gsl_sf_result *result ) {
 ...
 */
int gsl_sf_coulomb_CL_array( double lam_min, int kmax, double eta, double *cl ) {
 int k;
 gsl_sf_result cl_0;
...
coulomb_FG_series( const double lam, const double eta, const double x,
 ...
 while ( m < max_iter ) {
  ...
  if ( m > 15 ) {
   ...
   if ( max_abs_dA / ( max_abs_dA + abs_A ) < 4.0 * GSL_DBL_EPSILON && max_abs_dB / ( max_abs_dB + abs_B ) < 4.0 * GSL_DBL_EPSILON )
    break;
  }
...
coulomb_FG0_series( const double eta, const double x,
 ...
 while ( m < max_iter ) {
  ...
  if ( m > 15 ) {
   ...
   if ( max_abs_du / ( max_abs_du + abs_u ) < 40.0 * GSL_DBL_EPSILON && max_abs_dv / ( max_abs_dv + abs_v ) < 40.0 * GSL_DBL_EPSILON )
    break;
  }
...
coulomb_FGmhalf_series( const double eta, const double x,
 ...
 while ( m < max_iter ) {
  ...
  f_sum+= u_m;
  g_sum+= v_m;
  if ( f_sum != 0.0 && g_sum != 0.0 && ( fabs( u_m / f_sum ) + fabs( v_m / g_sum ) < 10.0 * GSL_DBL_EPSILON ) )
   break;
  u_mm2= u_mm1;
  u_mm1= u_m;
  v_mm2= v_mm1;
...
int gsl_sf_coulomb_wave_F_array( double lam_min, int kmax,
                                 double eta, double x,
                                 double *fc_array,
                                 double *F_exp ) {
 if ( x == 0.0 ) {
