...
int gsl_sf_bessel_Inu_scaled_asymp_unif_e( const double nu, const double x, gsl_sf_result *result ) {
 ...
 if ( stat_ex == GSL_SUCCESS ) {
  ...
  for ( i= 1; i < 16; i++ )
   tpow[i]= t * tpow[i - 1];
  sum= 1.0 + debye_u1( tpow ) / nu + debye_u2( tpow ) / ( nu * nu ) + debye_u3( tpow ) / ( nu * nu * nu ) + debye_u4( tpow ) / ( nu * nu * nu * nu ) + debye_u5( tpow ) / ( nu * nu * nu * nu * nu );
 ...
 */
int gsl_sf_bessel_JY_mu_restricted( const double mu, const double x,
...
int gsl_sf_bessel_J_CF1( const double nu, const double x,
 ...
 * using Gautschi (Euler) equivalent series.
 */
int gsl_sf_bessel_I_CF1_ser( const double nu, const double x, double *ratio ) {
 const int maxk= 20000;
 double tk= 1.0;
 double sum= 1.0;
 double rhok= 0.0;
...
int gsl_sf_bessel_JY_steed_CF2( const double nu, const double x,
                                double *P, double *Q ) {
 ...
 for ( i= 2; i <= max_iter; i++ ) {
  ...
  q= p * dli + q * dlr;
  p= temp;
  if ( fabs( dlr - 1.0 ) + fabs( dli ) < GSL_DBL_EPSILON )
   break;
 }
 ...
 */
int gsl_sf_bessel_K_scaled_steed_temme_CF2( const double nu, const double x,
 ...
 for ( i= 2; i <= maxiter; i++ ) {
  ...
  dels= Qi * delhi;
  s+= dels;
  if ( fabs( dels / s ) < GSL_DBL_EPSILON )
   break;
 }
