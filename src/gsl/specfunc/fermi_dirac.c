...
fd_neg( const double j, const double x, gsl_sf_result *result ) {
 ...
 if ( x < GSL_LOG_DBL_MIN ) {
  ...
  for ( jterm= 0; jterm <= itmax; jterm++ ) {
   ...
   xn+= x;
   if ( fabs( f - f_previous ) < fabs( f ) * 2.0 * GSL_DBL_EPSILON || xn < GSL_LOG_DBL_MIN )
    break;
   enx*= ex;
  }
...
fd_asymp( const double j, const double x, gsl_sf_result *result ) {
 ...
 for ( n= 1; n <= itmax; n++ ) {
  ...
  add= eta.val * xgam;
  if ( !j_integer && fabs( add ) > fabs( add_previous ) )
   break;
  if ( fabs( add / seqn_val ) < GSL_DBL_EPSILON )
   break;
  seqn_val+= add;
...
fd_series_int( const int j, const double x, gsl_sf_result *result ) {
 ...
 for ( n= 1; n <= j + 2; n++ ) {
  ...
  del= pow_factor * eta_factor.val;
  sum+= del;
  if ( fabs( del / sum ) < GSL_DBL_EPSILON )
   break;
 }
...
fd_UMseries_int( const int j, const double x, gsl_sf_result *result ) {
 ...
 for ( n= 1; n < nmax; n+= 2 ) {
  ...
  sum_odd_val+= del_val;
  sum_odd_err+= del_err;
  if ( fabs( del_val / sum_odd_val ) < GSL_DBL_EPSILON )
   break;
 }
 ...
 for ( n= 2; n < nmax; n+= 2 ) {
  ...
  if ( fabs( del_val / sum_even_val ) < GSL_DBL_EPSILON )
   break;
 }
