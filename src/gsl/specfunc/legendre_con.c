...
conicalP_negmu_xgt1_CF1( const double mu, const int ell, const double tau,
 ...
 for ( k= 1; k < maxk; k++ ) {
  ...
  tk*= rhok;
  sum+= tk;
  if ( fabs( tk / sum ) < GSL_DBL_EPSILON )
   break;
 }
...
inline static double olver_U1( double beta2, double p ) {
 return ( p - 1.0 ) / ( 24.0 * ( 1.0 + beta2 ) ) * ( 3.0 + beta2 * ( 2.0 + 5.0 * p * ( 1.0 + p ) ) );
}

inline static double olver_U2( double beta2, double p ) {
 ...
 * [Dunster, Proc. Roy. Soc. Edinburgh 119A, 311 (1991), p. 326]
 */
int gsl_sf_conicalP_xlt1_large_neg_mu_e( double mu, double tau, double x,
...
inline static double olver_B0_xi( double mu, double xi ) {
...
inline static double olver_B0_th( double mu, double theta ) {
 ...
 * [Olver, p. 469]
 */
int gsl_sf_conicalP_xgt1_neg_mu_largetau_e( const double mu, const double tau,
 ...
 * [Olver, p. 473]
 */
int gsl_sf_conicalP_xlt1_neg_mu_largetau_e( const double mu, const double tau,
...
conicalP_hyperg_large_x( const double mu, const double tau, const double y,
 ...
 for ( k= 1; k <= kmax; k++ ) {
  ...
  re_term= y / k * re_tmp;
  im_term= y / k * im_tmp;
  if ( fabs( re_term / asum ) < GSL_DBL_EPSILON && fabs( im_term / asum ) < GSL_DBL_EPSILON )
   break;
  re_sum+= re_term;
  im_sum+= im_term;
 }
 ...
 * x->Inf
 */
int gsl_sf_conicalP_large_x_e( const double mu, const double tau, const double x,
                               gsl_sf_result *result, double *ln_multiplier ) {
 /* 2F1 term
   */
 ...
 double lnpre_err= GSL_DBL_EPSILON * ( 0.5 * M_LN2 + 0.5 * M_LNPI ) + GSL_DBL_EPSILON * fabs( ( mu - 0.5 ) * lnx ) + GSL_DBL_EPSILON * fabs( 0.5 * mu ) * ( fabs( lnxp1 ) + fabs( lnxm1 ) );
...
int gsl_sf_conicalP_1_e( const double lambda, const double x, gsl_sf_result *result ) {
 ...
 if ( x <= -1.0 ) {
  ...
  if ( x < 1.0 ) {
   ...
   double bessterm= V0 * I0.val + V1 * I1.val;
   double besserr= fabs( V0 ) * I0.err + fabs( V1 ) * I1.err + 2.0 * GSL_DBL_EPSILON * fabs( V0 * I0.val ) + 2.0 * GSL_DBL_EPSILON * fabs( V1 * I1.val );
   double arg1= th * lambda;
   double sqts= sqrt( th / sth );
   ...
   const double besserr= fabs( V0 ) * J0.err + fabs( V1 ) * J1.err + 512.0 * 2.0 * GSL_DBL_EPSILON * fabs( V0 * J0.val ) + 512.0 * 2.0 * GSL_DBL_EPSILON * fabs( V1 * J1.val ) + GSL_DBL_EPSILON * fabs( xi_lam * V0 * J1.val ) + GSL_DBL_EPSILON * fabs( xi_lam * V1 * J0.val );
