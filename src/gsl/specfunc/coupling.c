...
inline static int locMax3( const int a, const int b, const int c ) {
 int d= GSL_MAX( a, b );
 return GSL_MAX( d, c );
}

inline static int locMin3( const int a, const int b, const int c ) {
 int d= GSL_MIN( a, b );
 return GSL_MIN( d, c );
}

inline static int locMin5( const int a, const int b, const int c, const int d, const int e ) {
 int f= GSL_MIN( a, b );
 int g= GSL_MIN( c, d );
 int h= GSL_MIN( f, g );
 return GSL_MIN( e, h );
...
m_selection_fails( int two_ja, int two_jb, int two_jc,
 ...
 return (
     abs( two_ma ) > two_ja || abs( two_mb ) > two_jb || abs( two_mc ) > two_jc || GSL_IS_ODD( two_ja + two_ma ) || GSL_IS_ODD( two_jb + two_mb ) || GSL_IS_ODD( two_jc + two_mc ) || ( two_ma + two_mb + two_mc ) != 0 );
}
...
int gsl_sf_coupling_3j_e( int two_ja, int two_jb, int two_jc,
 ...
 if ( two_ja < 0 || two_jb < 0 || two_jc < 0 ) {
  ...
  lnorm.val= 0.5 * ( bcn1.val + bcn2.val - bcd1.val - bcd2.val - bcd3.val - bcd4.val - log( two_jc + 1.0 ) );
  lnorm.err= 0.5 * ( bcn1.err + bcn2.err + bcd1.err + bcd2.err + bcd3.err + bcd4.err + GSL_DBL_EPSILON * log( two_jc + 1.0 ) );
