 ...
 if ( d_integer ) {
  ...
  if ( stat_ad2 == GSL_SUCCESS && stat_bd2 == GSL_SUCCESS ) {
   ...
   for ( j= 1; j < maxiter; j++ ) {
    ...
    if ( fabs( delta ) < GSL_DBL_EPSILON * fabs( sum2_val ) )
     break;
   }

   if ( j == maxiter )
    stat_F2= GSL_EMAXITER;
...
int gsl_sf_hyperg_2F1_e( double a, double b, const double c,
                         const double x,
                         gsl_sf_result *result ) {
 const double d= c - a - b;
 ...
 if ( fabs( x - 1.0 ) < locEPS && ( c - a - b ) > 0 && c != 0 && !c_neg_integer ) {
  ...
  if ( stat1 != GSL_SUCCESS || stat2 != GSL_SUCCESS || stat3 != GSL_SUCCESS || stat4 != GSL_SUCCESS ) {
   DOMAIN_ERROR( result );
  }
...
int gsl_sf_hyperg_2F1_conj_e( const double aR, const double aI, const double c,
                              const double x,
                              gsl_sf_result *result ) {
 ...
 if ( ( ax < 0.25 && fabs( aR ) < 20.0 && fabs( aI ) < 20.0 ) || ( c > 0.0 && x > 0.0 ) ) {
...
int gsl_sf_hyperg_2F1_renorm_e( const double a, const double b, const double c,
                                const double x,
                                gsl_sf_result *result ) {
 const double rinta= floor( a + 0.5 );
 const double rintb= floor( b + 0.5 );
...
int gsl_sf_hyperg_2F1_conj_renorm_e( const double aR, const double aI, const double c,
                                     const double x,
                                     gsl_sf_result *result ) {
 const double rintc= floor( c + 0.5 );
