...
hyperg_1F1_asymp_posx( const double a, const double b, const double x,
 ...
 if ( stat_a == GSL_SUCCESS && stat_b == GSL_SUCCESS ) {
  ...
  if ( stat_F == GSL_SUCCESS && F.val != 0 ) {
   ...
   double ln_term_err= 2.0 * GSL_DBL_EPSILON * ( fabs( a ) + fabs( b ) ) * fabs( lnx ) + 2.0 * GSL_DBL_EPSILON * fabs( a - b );
...
hyperg_1F1_CF1_p_ser( const double a, const double b, const double x, double *result ) {
  ...
  *result= 0.0;
  ...
  for ( k= 1; k < maxiter; k++ ) {
   ...
   pk*= rhok;
   sum+= pk;
   if ( fabs( pk / sum ) < 2.0 * GSL_DBL_EPSILON )
    break;
  }
  *result= a / ( b - x ) * sum;
  if ( k == maxiter )
...
hyperg_1F1_a_negint_lag( const int a, const double b, const double x, gsl_sf_result *result ) {
 ...
 if ( b < 0.0 ) {
  ...
  const double lnpre_err= lnfact.err + lng1.err + lng2.err + 2.0 * GSL_DBL_EPSILON * fabs( lnpre_val );
...
hyperg_1F1_U( const double a, const double b, const double x, gsl_sf_result *result ) {
 ...
 double lnc1_err= lg_2mbp.err + lg_1papmbp.err + GSL_DBL_EPSILON * ( fabs( lg_2mbp.val ) + fabs( lg_1papmbp.val ) );
 ...
 if ( Uee_val > Mee_val ) {
  ...
  const double inner_err=
      term_M.err * factorM_val + fabs( ombp ) * U.err + fabs( term_M.val ) * factorM_err + GSL_DBL_EPSILON * ( fabs( term_M.val * factorM_val ) + fabs( ombp * U.val ) );
  ...
  const double inner_err=
      term_M.err + fabs( ombp * factorU_val * U.err ) + fabs( ombp * factorU_err * U.val ) + GSL_DBL_EPSILON * ( fabs( term_M.val ) + fabs( ombp * factorU_val * U.val ) );
...
hyperg_1F1_ab_pos( const double a, const double b,
 ...
 if ( ( b < 10.0 && a < 10.0 && ax < 5.0 ) || ( b > a * ax ) || ( b > a && ax < 5.0 ) ) {
  return gsl_sf_hyperg_1F1_series_e( a, b, x, result );
 } else if ( x < -100.0 && GSL_MAX_DBL( fabs( a ), 1.0 ) * GSL_MAX_DBL( fabs( 1.0 + a - b ), 1.0 ) < 0.7 * fabs( x ) ) {
  /* Large negative x asymptotic.
     */
  return hyperg_1F1_asymp_negx( a, b, x, result );
 } else if ( x > 100.0 && GSL_MAX_DBL( fabs( b - a ), 1.0 ) * GSL_MAX_DBL( fabs( 1.0 - a ), 1.0 ) < 0.7 * fabs( x ) ) {
...
hyperg_1F1_ab_neg( const double a, const double b, const double x,
 ...
 if ( ( abs_a < 10.0 && abs_b < 10.0 && abs_x < 5.0 ) || ( b > 0.8 * GSL_MAX( fabs( a ), 1.0 ) * fabs( x ) ) ) {
  return gsl_sf_hyperg_1F1_series_e( a, b, x, result );
 } else if ( x > 0.0 && size_b > size_a && size_a * log( M_E * x / size_b ) < GSL_LOG_DBL_EPSILON + 7.0 ) {
  /* Series terms are positive definite up until
     ...
     */
  return gsl_sf_hyperg_1F1_series_e( a, b, x, result );
 } else if ( ( abs_x < 5.0 && fabs( bma ) < 10.0 && abs_b < 10.0 ) || ( b > 0.8 * GSL_MAX_DBL( fabs( bma ), 1.0 ) * abs_x ) ) {
  ...
  int stat_e= gsl_sf_exp_mult_err_e( x, GSL_DBL_EPSILON * fabs( x ),
                                     ...
                                     result );
  return GSL_ERROR_SELECT_2( stat_e, stat_K );
 } else if ( x < -30.0 && GSL_MAX_DBL( fabs( a ), 1.0 ) * GSL_MAX_DBL( fabs( 1.0 + a - b ), 1.0 ) < 0.99 * fabs( x ) ) {
  /* Large negative x asymptotic.
     * Note that we do not check if b-a is a negative integer.
     */
  return hyperg_1F1_asymp_negx( a, b, x, result );
 } else if ( x > 100.0 && GSL_MAX_DBL( fabs( bma ), 1.0 ) * GSL_MAX_DBL( fabs( 1.0 - a ), 1.0 ) < 0.99 * fabs( x ) ) {
  /* Large positive x asymptotic.
...
int gsl_sf_hyperg_1F1_int_e( const int a, const int b, const double x, gsl_sf_result *result ) {
...
int gsl_sf_hyperg_1F1_e( const double a, const double b, const double x,
                         gsl_sf_result *result ) {
 const double bma= b - a;
