...
void TYPE( gsl_sort )( BASE *data, const size_t stride, const size_t n ) {
 size_t N;
 size_t k;
...
void TYPE( gsl_sort_vector )( TYPE( gsl_vector ) * v ) {
 TYPE( gsl_sort )
 ( v->data, v->stride, v->size );
}

void TYPE( gsl_sort2 )( BASE *data1, const size_t stride1, BASE *data2, const size_t stride2, const size_t n ) {
 size_t N;
 size_t k;
 ...
 do {
  k--;
  FUNCTION( my, downheap2 )
  ( data1, stride1, data2, stride2, N, k );
 } while ( k > 0 );
 ...
 while ( N > 0 ) {
  ...
  FUNCTION( my, downheap2 )
  ( data1, stride1, data2, stride2, N, 0 );
 }
}

void TYPE( gsl_sort_vector2 )( TYPE( gsl_vector ) * v1, TYPE( gsl_vector ) * v2 ) {
 TYPE( gsl_sort2 )
 ( v1->data, v1->stride, v2->data, v2->stride, v1->size );
}
