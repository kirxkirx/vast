...
int gsl_integration_qng( const gsl_function *f,
...

 {
  ...
  for ( k= 0; k < 5; k++ ) {
   resasc+=
       ( w21a[k] * ( fabs( fv1[k] - mean ) + fabs( fv2[k] - mean ) ) + w21b[k] * ( fabs( fv3[k] - mean ) + fabs( fv4[k] - mean ) ) );
  }
 ...
 for ( k= 0; k < 11; k++ ) {
  ...
  const double fval= ( GSL_FN_EVAL( f, center + abscissa ) + GSL_FN_EVAL( f, center - abscissa ) );
  res43+= fval * w43b[k];
  savfun[k + 10]= fval;
 }
 ...
 for ( k= 0; k < 22; k++ ) {
  ...
  res87+= w87b[k] * ( GSL_FN_EVAL( f, center + abscissa ) + GSL_FN_EVAL( f, center - abscissa ) );
 }
