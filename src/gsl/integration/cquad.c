...
int gsl_integration_cquad( const gsl_function *f, double a, double b,
 ...
 while ( nivals > 0 && err > 0.0 &&
         !( err <= fabs( igral ) * epsrel || err <= epsabs ) && !( err_final > fabs( igral ) * epsrel && err - err_final < fabs( igral ) * epsrel ) && !( err_final > epsabs && err - err_final < epsabs ) ) {
...
*/
  ...
  if ( ( m + h * xi[0] ) >= ( m + h * xi[1] ) || ( m + h * xi[31] ) >= ( m + h * xi[32] ) || iv->err < fabs( iv->igral ) * GSL_DBL_EPSILON * 10 ) {
...
*/
  ...
  /* Do we need to split this interval? */
   ...
   ivl->ndiv= iv->ndiv + ( fabs( iv->c[0] ) > 0 && ivl->c[0] / iv->c[0] > 2 );
   ...
   /* Check for divergence. */
   ivr->ndiv= iv->ndiv + ( fabs( iv->c[0] ) > 0 && ivr->c[0] / iv->c[0] > 2 );
