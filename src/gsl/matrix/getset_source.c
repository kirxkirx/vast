...
int FUNCTION( gsl_matrix, get_row )( TYPE( gsl_vector ) * v,
...

 {
  ...
  for ( j= 0; j < N; j++ ) {
   ...
   for ( k= 0; k < MULTIPLICITY; k++ ) {
    v_data[MULTIPLICITY * stride * j + k]= row_data[MULTIPLICITY * j + k];
   }
  }
 }
...
int FUNCTION( gsl_matrix, get_col )( TYPE( gsl_vector ) * v,
                                     const TYPE( gsl_matrix ) * m,
                                     const size_t j ) {
 const size_t M= m->size1;
 const size_t N= m->size2;
...
int FUNCTION( gsl_matrix, set_row )( TYPE( gsl_matrix ) * m,
                                     const size_t i,
...

 {
  ...
  for ( j= 0; j < N; j++ ) {
   ...
   for ( k= 0; k < MULTIPLICITY; k++ ) {
    row_data[MULTIPLICITY * j + k]= v_data[MULTIPLICITY * stride * j + k];
   }
  }
 }
...
int FUNCTION( gsl_matrix, set_col )( TYPE( gsl_matrix ) * m,
                                     const size_t j,
                                     const TYPE( gsl_vector ) * v ) {
 const size_t M= m->size1;
 const size_t N= m->size2;
...

 {
  ...
  for ( i= 0; i < M; i++ ) {
   ...
   for ( k= 0; k < MULTIPLICITY; k++ ) {
    column_data[MULTIPLICITY * i * tda + k]= v_data[MULTIPLICITY * stride * i + k];
   }
  }
 }
