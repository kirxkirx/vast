...
int FUNCTION( gsl_matrix, memcpy )( TYPE( gsl_matrix ) * dest,
...
int FUNCTION( gsl_matrix, swap )( TYPE( gsl_matrix ) * dest, TYPE( gsl_matrix ) * src ) {
 const size_t src_size1= src->size1;
 const size_t src_size2= src->size2;
 const size_t dest_size1= dest->size1;
 const size_t dest_size2= dest->size2;
...

 {
  ...
  for ( i= 0; i < src_size1; i++ ) {
   for ( j= 0; j < MULTIPLICITY * src_size2; j++ ) {
    ...
    src->data[MULTIPLICITY * src_tda * i + j]= dest->data[MULTIPLICITY * dest_tda * i + j];
...
int FUNCTION( gsl_matrix, tricpy )( const char uplo_src,
