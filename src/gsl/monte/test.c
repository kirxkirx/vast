...
void add( struct problem *problems, int *n,
...
#define MONTE_INTEGRATE( f, xl, xu, dim, calls, r, s, res, err )      \
 {                                                                    \
  ...
  params.alpha= 1.5;                                                  \
  gsl_monte_miser_params_set( s, &params );                           \
...
#define MONTE_ERROR_TEST( err, expected )                                                                                                                                 \
 gsl_test( err > 3.0 * ( expected == 0 ? 1.0 / ( I->calls / MONTE_SPEEDUP ) : expected ), NAME ", %s, abserr[%d] (obs %g vs exp %g)", I->description, i, err, expected ); \
 gsl_test( gsl_monte_vegas_chisq( s ) < 0, NAME " returns valid chisq (%g)", gsl_monte_vegas_chisq( s ) )
#include "test_main.c"
#undef NAME
#undef MONTE_STATE
#undef MONTE_ALLOC
...
f2( double x[], size_t num_dim, void *params ) {
 ...
 return 0.5 * pow( M_2_SQRTPI / ( 2. * a ), num_dim ) * ( exp( -sum1 / ( a * a ) ) + exp( -sum2 / ( a * a ) ) );
}
