...
kirby2_f( const gsl_vector *x, void *params, gsl_vector *f ) {
 ...
 for ( i= 0; i < kirby2_N; i++ ) {
  ...
  double y= ( ( b[0] + t * ( b[1] + t * b[2] ) ) / ( 1 + t * ( b[3] + t * b[4] ) ) );
...
kirby2_fvv( const gsl_vector *x, const gsl_vector *v,
 ...
 for ( i= 0; i < kirby2_N; i++ ) {
  ...
  gsl_vector_set( fvv, i, -2 * t / pow( term1, 3.0 ) * ( v4 + t * v5 ) * ( -t * ( -v2 + v4 * x1 + t * ( -v3 + v5 * x1 + v4 * x2 + t * v5 * x2 + t * ( v4 + t * v5 ) * x3 ) ) + t * t * ( v2 + t * v3 ) * ( x4 + t * x5 ) + v1 * ( 1.0 + t * ( x4 + t * x5 ) ) ) );
 }
