...
void FUNCTION( test_complex, func )( size_t stride, size_t n ) {
...

 {
  ...
  gsl_test( cw == 0, NAME( gsl_fft_complex_wavetable ) "_alloc, n = %d, stride = %d", n, stride );
 }

 {
  cwork= FUNCTION( gsl_fft_complex_workspace, alloc )( n );
  gsl_test( cwork == 0, NAME( gsl_fft_complex_workspace ) "_alloc, n = %d", n );
 }

 /* Test mixed radix fft with noise */

 {
  FUNCTION( fft_signal, complex_noise )
  ( n, stride, complex_data, fft_complex_data );
  for ( i= 0; i < n; i++ ) {
   REAL( complex_tmp, stride, i )= REAL( complex_data, stride, i );
   IMAG( complex_tmp, stride, i )= IMAG( complex_data, stride, i );
  }

  FUNCTION( gsl_fft_complex, forward )
  ( complex_data, stride, n, cw, cwork );
  ...
  status= FUNCTION( compare_complex, results )( "dft", fft_complex_data,
                                                ...
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_forward with signal_noise, n = %d, stride = %d", n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( complex_data, stride, n, 0 );

   gsl_test( status, NAME( gsl_fft_complex ) "_forward avoids unstrided data, n = %d, stride = %d",
             n, stride );
  }
 }
...

 {
  ...
  status= FUNCTION( compare_complex, results )( "orig", complex_tmp,
                                                ...
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_inverse with signal_noise, n = %d, stride = %d", n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( complex_data, stride, n, 0 );

   gsl_test( status, NAME( gsl_fft_complex ) "_inverse other data untouched, n = %d, stride = %d",
             n, stride );
  }
 }
...

 {
  ...
  gsl_test( status, NAME( gsl_fft_complex ) "_backward with signal_noise, n = %d, stride = %d", n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( fft_complex_tmp, stride, n, 3000 );

   gsl_test( status, NAME( gsl_fft_complex ) "_backward avoids unstrided data, n = %d, stride = %d",
             n, stride );
  }
 }
 ...
 {
  FUNCTION( fft_signal, complex_pulse )
  ( 1, n, stride, 1.0, 0.0, complex_data,
    fft_complex_data );
  FUNCTION( gsl_fft_complex, forward )
  ( complex_data, stride, n, cw, cwork );
  status= FUNCTION( compare_complex, results )( "analytic", fft_complex_data,
                                                ...
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_forward with signal_pulse, n = %d, stride = %d", n, stride );
 }

 /* Test a constant signal */

 {
  FUNCTION( fft_signal, complex_constant )
  ( n, stride, 1.0, 0.0, complex_data,
    fft_complex_data );
  FUNCTION( gsl_fft_complex, forward )
  ( complex_data, stride, n, cw, cwork );
  status= FUNCTION( compare_complex, results )( "analytic", fft_complex_data,
                                                ...
                                                complex_data,
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_forward with signal_constant, n = %d, stride = %d", n, stride );
 }
...

 {
  status= 0;
  for ( i= 0; i < n; i++ ) {
   FUNCTION( fft_signal, complex_exp )
   ( (int)i, n, stride, 1.0, 0.0, complex_data,
     fft_complex_data );
   FUNCTION( gsl_fft_complex, forward )
   ( complex_data, stride, n, cw, cwork );
  ...
  gsl_test( status, NAME( gsl_fft_complex ) "_forward with signal_exp, n = %d, stride = %d", n, stride );
 }

 FUNCTION( gsl_fft_complex_wavetable, free )
 ( cw );
 FUNCTION( gsl_fft_complex_workspace, free )
 ( cwork );
...
void FUNCTION( test_complex, bitreverse_order )( size_t stride, size_t n ) {
 ...
 FUNCTION( fft_complex, bitreverse_order )
 ( data, stride, n, logn );
...
void FUNCTION( test_complex, radix2 )( size_t stride, size_t n ) {
...

 {
  ...
  FUNCTION( gsl_fft_complex, radix2_forward )
  ( complex_data, stride, n );
  ...
  status= FUNCTION( compare_complex, results )( "dft", fft_complex_data,
                                                "fft of noise", complex_data,
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_forward with signal_noise, n = %d, stride = %d",
            n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( complex_data, stride, n, 0 );

   gsl_test( status, NAME( gsl_fft_complex ) "_radix2_forward avoids unstrided data, n = %d, stride = %d",
             n, stride );
  }
 }
...

 {
  ...
  status= FUNCTION( compare_complex, results )( "orig", complex_tmp,
                                                "fft inverse", complex_data,
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_inverse with signal_noise, n = %d, stride = %d", n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( complex_data, stride, n, 0 );

   gsl_test( status, NAME( gsl_fft_complex ) "_radix2_inverse other data untouched, n = %d, stride = %d",
             n, stride );
  }
 }
...

 {
  ...
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_backward with signal_noise, n = %d, stride = %d", n, stride );

  if ( stride > 1 ) {
   status= FUNCTION( test, offset )( fft_complex_tmp, stride, n, 3000 );

   gsl_test( status, NAME( gsl_fft_complex ) "_radix2_backward avoids unstrided data, n = %d, stride = %d",
             n, stride );
  }
 }
...

 {
  ...
  status= FUNCTION( compare_complex, results )( "analytic", fft_complex_data,
                                                "fft of pulse", complex_data,
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_forward with signal_pulse, n = %d, stride = %d", n, stride );
 }
...

 {
  ...
  status= FUNCTION( compare_complex, results )( "analytic", fft_complex_data,
                                                ...
                                                complex_data,
                                                stride, n, 1e6 );
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_forward with signal_constant, n = %d, stride = %d",
            n, stride );
 }
...

 {
  ...
  gsl_test( status, NAME( gsl_fft_complex ) "_radix2_forward with signal_exp, n = %d, stride = %d", n, stride );
 }
