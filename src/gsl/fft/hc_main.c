...
int FUNCTION( gsl_fft_halfcomplex, backward )( BASE data[], const size_t stride,
                                               const size_t n,
...
int FUNCTION( gsl_fft_halfcomplex, inverse )( BASE data[], const size_t stride,
                                              const size_t n,
                                              const TYPE( gsl_fft_halfcomplex_wavetable ) * wavetable,
                                              TYPE( gsl_fft_real_workspace ) * work ) {
 int status= FUNCTION( gsl_fft_halfcomplex, transform )( data, stride, n, wavetable, work );
...
int FUNCTION( gsl_fft_halfcomplex, transform )( BASE data[], const size_t stride, const size_t n,
                                                const TYPE( gsl_fft_halfcomplex_wavetable ) * wavetable,
                                                TYPE( gsl_fft_real_workspace ) * work ) {
 ...
 if ( n == 1 ) { /* FFT of one data point is the identity */
  return 0;
 }
 ...
 for ( i= 0; i < nf; i++ ) {
  ...
  if ( factor == 2 ) {
   ...
   FUNCTION( fft_halfcomplex, pass_2 )
   ( in, istride, out, ostride,
     product, n, twiddle1 );
  } else if ( factor == 3 ) {
   twiddle1= wavetable->twiddle[i];
   twiddle2= twiddle1 + tskip;
   FUNCTION( fft_halfcomplex, pass_3 )
   ( in, istride, out, ostride,
   ...
   FUNCTION( fft_halfcomplex, pass_4 )
   ( in, istride, out, ostride,
     product, n, twiddle1, twiddle2,
     twiddle3 );
  } else if ( factor == 5 ) {
   ...
   FUNCTION( fft_halfcomplex, pass_5 )
   ( in, istride, out, ostride,
     product, n, twiddle1, twiddle2,
     twiddle3, twiddle4 );
  } else {
   twiddle1= wavetable->twiddle[i];
   FUNCTION( fft_halfcomplex, pass_n )
   ( in, istride, out, ostride,
