...
int FUNCTION( gsl_fft_complex, forward )( TYPE( gsl_complex_packed_array ) data,
                                          const size_t stride,
                                          const size_t n,
...
int FUNCTION( gsl_fft_complex, backward )( TYPE( gsl_complex_packed_array ) data,
                                           const size_t stride,
                                           const size_t n,
                                           const TYPE( gsl_fft_complex_wavetable ) * wavetable,
                                           TYPE( gsl_fft_complex_workspace ) * work ) {
...
int FUNCTION( gsl_fft_complex, inverse )( TYPE( gsl_complex_packed_array ) data,
                                          const size_t stride,
                                          const size_t n,
                                          const TYPE( gsl_fft_complex_wavetable ) * wavetable,
                                          TYPE( gsl_fft_complex_workspace ) * work ) {
...
int FUNCTION( gsl_fft_complex, transform )( TYPE( gsl_complex_packed_array ) data,
                                            const size_t stride,
                                            const size_t n,
                                            const TYPE( gsl_fft_complex_wavetable ) * wavetable,
                                            TYPE( gsl_fft_complex_workspace ) * work,
 ...
 for ( i= 0; i < nf; i++ ) {
  ...
  if ( factor == 2 ) {
   ...
   FUNCTION( fft_complex, pass_2 )
   ( in, istride, out, ostride, sign,
     product, n, twiddle1 );
  } else if ( factor == 3 ) {
   twiddle1= wavetable->twiddle[i];
   twiddle2= twiddle1 + q;
   FUNCTION( fft_complex, pass_3 )
   ( in, istride, out, ostride, sign,
