...
lm_iteration:
 ...
 printf( "perm = " );
 gsl_permutation_fprintf( stdout, perm, "%d" );
 printf( "dx = " );
 gsl_vector_fprintf( stdout, dx, "%g" );
#endif
...
#ifdef DEBUG
 printf( "rptdx = " );
 gsl_vector_fprintf( stdout, rptdx, "%g" );
#endif
 ...
 if ( ratio >= p0001 ) {
  ...
  return GSL_SUCCESS;
 } else if ( fabs( actred ) <= GSL_DBL_EPSILON && prered <= GSL_DBL_EPSILON && p5 * ratio <= 1.0 ) {
  return GSL_ETOLF;
