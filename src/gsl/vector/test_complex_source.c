...
void FUNCTION( test, func )( size_t stride, size_t N ) {
 ...
 if ( stride == 1 ) {
  ...
  FUNCTION( gsl_vector, free )
  ( v ); /* free whatever is in v */
 }
...

 {
...

  {
   ...
   GSL_REAL( x )= 7.0;
   GSL_IMAG( x )= 13.0;
   FUNCTION( gsl_vector, add_constant )
   ( v, x );
  }
...

 {
  ...
  FUNCTION( gsl_vector, swap_elements )
  ( v, 2, 5 );
...

 {
  ...
  QUALIFIED_VIEW( gsl_vector, view )
  v1= FUNCTION( gsl_vector, view_array )( v->data, N * stride );
...

 {
  ...
  QUALIFIED_VIEW( gsl_vector, view )
  v1= FUNCTION( gsl_vector, view_array_with_stride )( v->data, stride, N * stride );
...

 {
  ...
  QUALIFIED_VIEW( gsl_vector, view )
  v1= FUNCTION( gsl_vector, subvector )( v, N / 3, N / 2 );
...

 {
  ...
  QUALIFIED_VIEW( gsl_vector, view )
  v1= FUNCTION( gsl_vector, subvector_with_stride )( v, N / 5, 3, N / 4 );
...
void FUNCTION( test, ops )( size_t stride1, size_t stride2, size_t N ) {
...

 {
  ...
  for ( i= 0; i < N; i++ ) {
   BASE r= FUNCTION( gsl_vector, get )( v, i );
   if ( GSL_REAL( r ) != ( ATOMIC )( 3 * i + 11 ) || GSL_IMAG( r ) != ( ATOMIC )( 3 * i + 36 ) )
    status= 1;
  }
...
void FUNCTION( test, file )( size_t stride, size_t N ) {
 ...
 FUNCTION( gsl_vector, fwrite )
 ( f, v );

 /* read file */

 rewind( f );
 FUNCTION( gsl_vector, fread )
 ( f, w );
...
void FUNCTION( test, text )( size_t stride, size_t N ) {
 ...
 FUNCTION( gsl_vector, fprintf )
 ( f, v, OUT_FORMAT );

 /* read file */

 rewind( f );
 FUNCTION( gsl_vector, fscanf )
 ( f, w );
