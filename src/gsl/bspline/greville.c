...
int gsl_bspline_knots_greville( const gsl_vector *abscissae,
 ...
 if ( w->nbreak == 2 ) {
  ...
  s= gsl_linalg_QR_decomp( &A.matrix, &tau.vector ) || gsl_linalg_QR_lssolve( &A.matrix, &tau.vector,
  ...
  *abserr= 0;
   ...
   *abserr+= fabs( gsl_bspline_greville_abscissa( i, w ) - gsl_vector_get( abscissae, i ) );
 }
