...
int gsl_bspline_deriv_eval_nonzero( const double x, const size_t nderiv,
...
bspline_find_interval( const double x, int *flag, gsl_bspline_workspace *w ) {
 ...
 for ( i= w->k - 1; i < w->k + w->l - 1; i++ ) {
  ...
  if ( ti < x && x == tip1 && tip1 == gsl_vector_get( w->knots, w->k + w->l - 1 ) )
   break;
 }
...
bspline_pppack_bsplvb( const gsl_vector *t,
 ...
 for ( /* NOP */; *j < jhigh - 1; *j+= 1 ) {
  ...
  for ( i= 0; i <= *j; i++ ) {
   term= gsl_vector_get( biatx, i ) / ( gsl_vector_get( deltar, i ) + gsl_vector_get( deltal,
                                                                                      *j - i ) );
 ...
 if ( mhigh > 0 ) {
  ...
  for ( m= 1; m <= mhigh; m++ ) {
   ...
   for ( ldummy= 0; ldummy < kmm; ldummy++ ) {
    ...
    for ( j= 0; j <= i; j++ ) {
     gsl_matrix_set( a, i, j, factor * ( gsl_matrix_get( a, i, j ) - gsl_matrix_get( a, i - 1, j ) ) );
    }
    il--;
    i--;
   }
