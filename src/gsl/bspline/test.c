...
void test_bspline( gsl_bspline_workspace *bw ) {
 gsl_vector *B;
 gsl_matrix *dB;
 size_t i, j;
 size_t n= 100;
...
int main( int argc, char **argv ) {
 ...
 /* Knots computed from prescribed Greville abscissae for k = 4 */
 {
  ...
  double abserr;
  gsl_vector_const_view abscissae= gsl_vector_const_view_array( abscissae_data, nabscissae );
  gsl_bspline_workspace *w= gsl_bspline_alloc( k, nbreak );
 ...
 /* Knots computed from prescribed abscissae for edge case when nbreak = 2 */
 {
  ...
  double abserr;
  gsl_vector_view abscissae= gsl_vector_view_array( abscissae_data, nabscissae );
  gsl_bspline_workspace *w= gsl_bspline_alloc( k, nbreak );
  gsl_bspline_knots_greville( &abscissae.vector, w, &abserr );
