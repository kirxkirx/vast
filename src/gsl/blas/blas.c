...
int gsl_blas_sdsdot( float alpha, const gsl_vector_float *X,
...
int gsl_blas_sdot( const gsl_vector_float *X, const gsl_vector_float *Y,
                   float *result ) {
 if ( X->size == Y->size ) {
  *result=
...
int gsl_blas_zaxpy( const gsl_complex alpha, const gsl_vector_complex *X,
...
int gsl_blas_sgemv( CBLAS_TRANSPOSE_t TransA, float alpha,
                    const gsl_matrix_float *A, const gsl_vector_float *X,
                    float beta, gsl_vector_float *Y ) {
 const size_t M= A->size1;
 const size_t N= A->size2;

 if ( ( TransA == CblasNoTrans && N == X->size && M == Y->size ) || ( TransA == CblasTrans && M == X->size && N == Y->size ) ) {
...
int gsl_blas_cgemv( CBLAS_TRANSPOSE_t TransA, const gsl_complex_float alpha,
 ...
 if ( ( TransA == CblasNoTrans && N == X->size && M == Y->size ) || ( TransA == CblasTrans && M == X->size && N == Y->size ) || ( TransA == CblasConjTrans && M == X->size && N == Y->size ) ) {
...
int gsl_blas_chemv( CBLAS_UPLO_t Uplo, const gsl_complex_float alpha,
                    const gsl_matrix_complex_float *A,
                    const gsl_vector_complex_float *X,
                    const gsl_complex_float beta, gsl_vector_complex_float *Y ) {
 const size_t M= A->size1;
...
int gsl_blas_ssymv( CBLAS_UPLO_t Uplo, float alpha, const gsl_matrix_float *A,
...
int gsl_blas_dsymv( CBLAS_UPLO_t Uplo, double alpha, const gsl_matrix *A,
                    const gsl_vector *X, double beta, gsl_vector *Y ) {
 const size_t M= A->size1;
 const size_t N= A->size2;
...
int gsl_blas_sger( float alpha, const gsl_vector_float *X,
...
int gsl_blas_cher( CBLAS_UPLO_t Uplo, float alpha,
                   const gsl_vector_complex_float *X,
                   gsl_matrix_complex_float *A ) {
 const size_t M= A->size1;
 const size_t N= A->size2;
...
int gsl_blas_cher2( CBLAS_UPLO_t Uplo, const gsl_complex_float alpha,
                    const gsl_vector_complex_float *X,
                    const gsl_vector_complex_float *Y,
                    gsl_matrix_complex_float *A ) {
 const size_t M= A->size1;
...
int gsl_blas_ssyr( CBLAS_UPLO_t Uplo, float alpha, const gsl_vector_float *X,
                   gsl_matrix_float *A ) {
 const size_t M= A->size1;
 const size_t N= A->size2;
...
int gsl_blas_dsyr( CBLAS_UPLO_t Uplo, double alpha, const gsl_vector *X,
                   gsl_matrix *A ) {
 const size_t M= A->size1;
 const size_t N= A->size2;
...
int gsl_blas_ssyr2( CBLAS_UPLO_t Uplo, float alpha, const gsl_vector_float *X,
                    const gsl_vector_float *Y, gsl_matrix_float *A ) {
 const size_t M= A->size1;
 const size_t N= A->size2;
...
int gsl_blas_dsyr2( CBLAS_UPLO_t Uplo, double alpha, const gsl_vector *X,
...
int gsl_blas_zgemm( CBLAS_TRANSPOSE_t TransA, CBLAS_TRANSPOSE_t TransB,
...
int gsl_blas_ssymm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo, float alpha,
 ...
 if ( ( Side == CblasLeft && ( M == MA && N == NB && NA == MB ) ) || ( Side == CblasRight && ( M == MB && N == NA && NB == MA ) ) ) {
...
int gsl_blas_dsymm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo, double alpha,
...
int gsl_blas_chemm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    const gsl_complex_float alpha,
                    const gsl_matrix_complex_float *A,
                    const gsl_matrix_complex_float *B,
                    const gsl_complex_float beta, gsl_matrix_complex_float *C ) {
...
int gsl_blas_zhemm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    const gsl_complex alpha, const gsl_matrix_complex *A,
                    const gsl_matrix_complex *B, const gsl_complex beta,
                    gsl_matrix_complex *C ) {
 const size_t M= C->size1;
...
int gsl_blas_ssyrk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, float alpha,
...
int gsl_blas_dsyrk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, double alpha,
...
int gsl_blas_csyrk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans,
                    const gsl_complex_float alpha,
                    const gsl_matrix_complex_float *A,
                    const gsl_complex_float beta, gsl_matrix_complex_float *C ) {
 const size_t M= C->size1;
...
int gsl_blas_zsyrk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans,
                    const gsl_complex alpha, const gsl_matrix_complex *A,
...
int gsl_blas_cherk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, float alpha,
...
int gsl_blas_zherk( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, double alpha,
...
int gsl_blas_ssyr2k( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, float alpha,
                     const gsl_matrix_float *A, const gsl_matrix_float *B,
                     float beta, gsl_matrix_float *C ) {
 const size_t M= C->size1;
 const size_t N= C->size2;
...
int gsl_blas_dsyr2k( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans, double alpha,
                     const gsl_matrix *A, const gsl_matrix *B, double beta,
                     gsl_matrix *C ) {
 const size_t M= C->size1;
 const size_t N= C->size2;
...
int gsl_blas_csyr2k( CBLAS_UPLO_t Uplo, CBLAS_TRANSPOSE_t Trans,
                     const gsl_complex_float alpha,
                     const gsl_matrix_complex_float *A,
                     const gsl_matrix_complex_float *B,
                     const gsl_complex_float beta, gsl_matrix_complex_float *C ) {
...
int gsl_blas_strmm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
...
int gsl_blas_dtrmm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
...
int gsl_blas_ctrmm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
...
int gsl_blas_ztrmm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag,
                    const gsl_complex alpha, const gsl_matrix_complex *A,
...
int gsl_blas_strsm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag, float alpha,
                    const gsl_matrix_float *A, gsl_matrix_float *B ) {
 const size_t M= B->size1;
 const size_t N= B->size2;
...
int gsl_blas_dtrsm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag, double alpha,
                    const gsl_matrix *A, gsl_matrix *B ) {
 const size_t M= B->size1;
 const size_t N= B->size2;
...
int gsl_blas_ctrsm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag,
                    const gsl_complex_float alpha,
                    const gsl_matrix_complex_float *A,
                    gsl_matrix_complex_float *B ) {
...
int gsl_blas_ztrsm( CBLAS_SIDE_t Side, CBLAS_UPLO_t Uplo,
                    CBLAS_TRANSPOSE_t TransA, CBLAS_DIAG_t Diag,
                    const gsl_complex alpha, const gsl_matrix_complex *A,
                    gsl_matrix_complex *B ) {
 const size_t M= B->size1;
