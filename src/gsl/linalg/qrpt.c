...
int gsl_linalg_QRPT_lssolve2( const gsl_matrix *QR, const gsl_vector *tau, const gsl_permutation *p,
...
int gsl_linalg_QRPT_QRsolve( const gsl_matrix *Q, const gsl_matrix *R,
                             const gsl_permutation *p,
                             const gsl_vector *b,
                             gsl_vector *x ) {
 if ( Q->size1 != Q->size2 || R->size1 != R->size2 ) {
  return GSL_ENOTSQR;
 } else if ( Q->size1 != p->size || Q->size1 != R->size1 || Q->size1 != b->size ) {
  return GSL_EBADLEN;
 } else {
  /* compute b' = Q^T b */
...
int gsl_linalg_QRPT_update( gsl_matrix *Q, gsl_matrix *R,
                            const gsl_permutation *p,
