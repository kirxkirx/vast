...
int TYPE( gsl_permute )( const size_t *p, ATOMIC *data, const size_t stride, const size_t n ) {
 size_t i, k, pk;
...
int FUNCTION( gsl_permute, inverse )( const size_t *p, ATOMIC *data, const size_t stride, const size_t n ) {
 size_t i, k, pk;
...
int TYPE( gsl_permute_vector )( const gsl_permutation *p, TYPE( gsl_vector ) * v ) {
 if ( v->size != p->size ) {
  GSL_ERROR( "vector and permutation must be the same length", GSL_EBADLEN );
 }

 TYPE( gsl_permute )
 ( p->data, v->data, v->stride, v->size );

 return GSL_SUCCESS;
}

int FUNCTION( gsl_permute_vector, inverse )( const gsl_permutation *p, TYPE( gsl_vector ) * v ) {
 if ( v->size != p->size ) {
  GSL_ERROR( "vector and permutation must be the same length", GSL_EBADLEN );
 }

 FUNCTION( gsl_permute, inverse )
 ( p->data, v->data, v->stride, v->size );

 return GSL_SUCCESS;
}

int TYPE( gsl_permute_matrix )( const gsl_permutation *p, TYPE( gsl_matrix ) * A ) {
 if ( A->size2 != p->size ) {
  ...
  for ( i= 0; i < A->size1; ++i ) {
   ...
   r= FUNCTION( gsl_matrix, row )( A, i );

   TYPE( gsl_permute_vector )
   ( p, &r.vector );
  }
