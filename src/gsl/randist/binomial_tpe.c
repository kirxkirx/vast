...
Stirling( double y1 ) {
 ...
 double s=
     ( 13860.0 -
       ( 462.0 - ( 132.0 - ( 99.0 - 140.0 / y2 ) / y2 ) / y2 ) / y2 ) /
     y1 / 166320.0;
 return s;
}
...
gsl_ran_binomial( const gsl_rng *rng, double p, unsigned int n ) {
 ...
 TryAgain:
  ...
  accept=
      LNFACT( m ) + LNFACT( n - m ) - LNFACT( ix ) - LNFACT( n - ix ) + ( ix - m ) * log( p / q );
  ...
  if ( k <= FAR_FROM_MEAN ) {
...

   {
    ...
    accept= xm * log( f1 / x1 ) + ( n - m + 0.5 ) * log( z1 / w1 ) + ( ix - m ) * log( w1 * p / ( x1 * q ) ) + Stirling( f1 ) + Stirling( z1 ) - Stirling( x1 ) - Stirling( w1 );
   }
#endif
#endif
 }
