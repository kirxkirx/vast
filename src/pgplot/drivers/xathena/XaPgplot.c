 ...
 */
 if ( request->request_mode & CWWidth ) {
  reply->width= request->width < XAP_MIN_WIDTH ? XAP_MIN_WIDTH : request->width;
 };
 /*
 * Check the requested height.
 */
 if ( request->request_mode & CWHeight ) {
  reply->height= request->height < XAP_MIN_HEIGHT ? XAP_MIN_HEIGHT : request->height;
 };
 /*
 ...
 */
#ifdef VMS
void xadriv( ifunc, rbuf, nbuf, chrdsc, lchr ) int *ifunc;
float rbuf[];
int *nbuf;
...
#else
void XADRIV( ifunc, rbuf, nbuf, chr, lchr, len ) int *ifunc, *nbuf, *lchr;
int len;
float rbuf[];
char *chr;
{
