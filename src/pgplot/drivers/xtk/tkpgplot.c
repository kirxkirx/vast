 ...
 */
   tkpg_RemoveWidget( pgx->name, pgx->state ? TKPG_ACTIVE_WIDGETS : TKPG_FREE_WIDGETS );
   /*
 ...
 */
#ifdef VMS
void DRIV( ifunc, rbuf, nbuf, chrdsc, lchr ) int *ifunc;
float rbuf[];
int *nbuf;
...
#else
void DRIV( ifunc, rbuf, nbuf, chr, lchr, len ) int *ifunc, *nbuf, *lchr;
int len;
float rbuf[];
char *chr;
{
 ...
 */
 if ( tkpg->highlight_thickness != 0 ) {
  GC gc= Tk_GCForColor( tkpg->events.focus_acquired ? tkpg->highlightColor : tkpg->highlightBgColor,
                        w );
