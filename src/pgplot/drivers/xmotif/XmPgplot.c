 ...
 */
 {
  ...
  pgx->expose_gc= XCreateGC( pgx->display, pgx->window, (unsigned long)( GCGraphicsExposures ), &gcv );
 ...
 */
 if ( request->request_mode & CWWidth ) {
  reply->width= request->width < XMP_MIN_WIDTH ? XMP_MIN_WIDTH : request->width;
 };
 /*
 * Check the requested height.
 */
 if ( request->request_mode & CWHeight ) {
  reply->height= request->height < XMP_MIN_HEIGHT ? XMP_MIN_HEIGHT : request->height;
 };
 /*
 ...
 */
#ifdef VMS
void xmdriv( ifunc, rbuf, nbuf, chrdsc, lchr ) int *ifunc;
float rbuf[];
int *nbuf;
...
#else
void XMDRIV( ifunc, rbuf, nbuf, chr, lchr, len ) int *ifunc, *nbuf, *lchr;
int len;
float rbuf[];
char *chr;
{
 ...
 */
  XtVaSetValues( xmp->scroll.w_hbar,
                 ...
                 XmNmaximum, scroll->width > w->core.width ? scroll->width : w->core.width,
 ...
 */
  XtVaSetValues( xmp->scroll.w_vbar,
                 ...
                 XmNmaximum, scroll->height > w->core.height ? scroll->height : w->core.height,
 ...
 */
 XtSetArg( scroll_args[scroll_count], XmNscrollingPolicy,
           (XtArgVal)XmAPPLICATION_DEFINED );
 scroll_count++;
 ...
 XtSetArg( scroll_args[scroll_count], XmNscrollBarDisplayPolicy,
           (XtArgVal)XmSTATIC );
 scroll_count++;
 XtSetArg( scroll_args[scroll_count], XmNshadowThickness, (XtArgVal)0 );
 scroll_count++;
 /*
