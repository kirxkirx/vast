...
#ifdef VMS
static unsigned short ntohs( netshort ) unsigned short netshort;
{
...
void figdisp_sendcommand( buffer, len ) short *buffer; /* the command buffer */
...
short *figdisp_getresponse( len ) int *len; /* the number of shorts in the response */
{
 ...
 while ( fd_dispopen && !xerror ) {
  ...
  if ( event.xproperty.atom == selatom &&
   ...
   if ( XGetWindowProperty( display, window, selatom, 0L, 10L,
                            ...
                            &bytesleft, (unsigned char **)&retval ) != Success ) {
    *len= 0;
    retval= (short *)NULL;
   } else {
    if ( bytesleft )
...
int figdisp_opencomm( maxbuf, dev ) int maxbuf;
...
void figdisp_convbufout( buf, len ) unsigned short *buf; /* The command buffer to convert */
...
void figdisp_convbufin( buf, len ) unsigned short *buf; /* The command buffer to convert */
int len;                                                /* The number of shorts in the command buffer. */
{
 unsigned short testshort= 0x1234;
 unsigned short realcom;
