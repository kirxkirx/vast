...
#ifdef VMS
void x2driv( opcode, rbuf, nbuf, chrdsc, lchr ) int *opcode; /* The specific PGPLOT function */
...
#else
void X2DRIV( opcode, rbuf, nbuf, chr, lchr, chrlen ) int *opcode; /* The specific PGPLOT function */
float *rbuf;                                                      /* the floating point values */
int *nbuf;                                                        /* number of floats in rbuf */
...
int chrlen;                                                       /* actual fortran length of chr */
{
 ...
 itmp= 0;
 switch ( *opcode ) { /* take care of the trivial cases without consulting X */
 case 1:
 ...
 switch ( *opcode ) { /* do the real work */
 case 2:
 ...
 case 18:
  break;
 case 9:
  if ( isopen ) { /* already open: report an error */
 ...
 case 17:
  ...
  if ( answer[3] & 0xFF00 ) { /* it was a button press */
 ...
 case 20:
  ...
  if ( !npolypts ) {
   ...
   if ( ( polypts= (short *)malloc(
              (unsigned)2 * npolypts * sizeof( short ) ) ) == (short *)NULL ) {
