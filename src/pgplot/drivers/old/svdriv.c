...
void svdriv_( FUNC, BUFFER, NBUF, STRING, NSTR, len ) int *FUNC; /* function */
 ...
 */
  ...
  *NBUF= 2;
  ...
  if ( i == 2 ) {
   ...
   if ( REQ_X_WID < MIN_X_WID )
    REQ_X_WID= DEF_X_WID;
   if ( REQ_X_WID > MAX_X_WID )
    REQ_X_WID= MAX_X_WID - MIN_X_WID;
   if ( REQ_Y_WID < MIN_Y_WID )
    REQ_Y_WID= DEF_Y_WID;
   if ( REQ_Y_WID > MAX_Y_WID )
    REQ_Y_WID= MAX_Y_WID - MIN_Y_WID;
   X_WID= REQ_X_WID;
   Y_WID= REQ_Y_WID;
  }
...
static update( sig, code, scp ) int sig, code;
...
GR_rectangle( x_lo, y_lo, x_hi, y_hi, color ) int x_lo, y_lo, x_hi, y_hi, color;
{
 pw_rop( pw, x_lo, FLIPY( y_hi ), x_hi - x_lo, y_hi - y_lo,
         PIX_SRC | PIX_COLOR( color ), 0, 0, 0 );
}

GR_vector( x1, y1, x2, y2, color ) int x1, y1, x2, y2, color;
{
 pw_vector( pw, x1, FLIPY( y1 ), x2, FLIPY( y2 ),
            PIX_SRC | PIX_COLOR( color ), 0 );
}

GR_poly( npoly, vertex, color ) int npoly, color;
int *vertex;
{
 int i;
...
unsigned char GR_cursor( x, y, string ) int *x, *y;
char *string;
{
...
caddr_t arg;
{
 ...
 if ( event_is_ascii( event ) ) /* keyboard */
  c_cursor= id;
 else if ( event_is_button( event ) ) { /* mouse keys */
