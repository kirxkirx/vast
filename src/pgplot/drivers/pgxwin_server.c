...
#else
static Bool xw_parse_bool( str, def ) char *str;
Bool def;
#endif
{
 /*
...
#else
static int xw_parse_visual( str ) char *str;
#endif
{
 /*
...
#else
static int xw_same_string( s1, s2 ) char *s1;
char *s2;
#endif
{
 /*
 ...
 */
 {
  ...
  xw_sync_error( xw );
  pgw->gc= XCreateGC( xw->display, pgw->window, (unsigned long)( GCGraphicsExposures ), &gcv );
 };
 ...
 */
 xw_sync_error( xw );
 XSelectInput( xw->display, pgw->client, (long)( StructureNotifyMask | PropertyChangeMask ) );
 if ( xw_sync_error( xw ) ) {
  fprintf( stderr,
 ...
 */
  do {
   XWindowEvent( xw->display, pgw->client, (long)( StructureNotifyMask | PropertyChangeMask ), &event );
   /*
