...
int *go_on; /* whether the calling routine shoudl exit successfully */
{
 ...
 case SelectionClear:
  ...
  if ( event.xselectionclear.selection != selatom )
   break;

  /* someone's grabbed the selection, get ready for some data */
  if ( !selset ) {
   if ( ( srcwin= XGetSelectionOwner( display, selatom ) ) == None ) {
  ...
  *go_on= getdata( event.xselection, &retbuf[0], &retbuflen,
  ...
  break;
 case Expose:
  if ( event.xexpose.window == lg.win )
   exposelgwin( event.xexpose );
#ifndef PGDISP
  else if ( event.xexpose.window == bm.win )
   exposebmwin( event.xexpose.x, event.xexpose.y,
                event.xexpose.width, event.xexpose.height );
  else if ( event.xexpose.window == patch.win )
   exposepatch();
  else if ( event.xexpose.window == box.win )
   exposebox();
  ...
  else if ( event.xexpose.window == seeing.win )
   exposesee();
#endif
  break;
 ...
 case ButtonRelease:
  if ( event.xbutton.window == bm.win ) {
   if ( event.xbutton.button == Button3 ) {
    ...
    slitxe= slitxs= -1;
    xorlinedrawn= 0;
   } else if ( event.xbutton.button == Button1 && modluttransoff ) {
    modluttransoff= 0;
 ...
 case MotionNotify:
  if ( event.xbutton.window == lg.win ) {
   ...
   while ( XCheckMaskEvent( display, PointerMotionMask,
                            &event2 ) != False ) {
    if ( event2.xbutton.window != lg.win )
     break;
    px= event2.xbutton.x;
    py= event2.xbutton.y;
   }
  ...
  if ( modluttransoff ) {
   modluttransoff= 1;
   if ( bppix == 16 )
    luttransoff= ( 65536 * ( event.xbutton.x - (int)bm.width / 2 ) ) / ( (int)bm.width / 2 );
   else
    luttransoff= ( 256 * ( event.xbutton.x - (int)bm.width / 2 ) ) / ( (int)bm.width / 2 );
   transluts();
  }
  if ( bm.modlut ) {
   double scale;
  ...
  if ( slitxs > 0 ) {
   /* erase old line */
   if ( xorlinedrawn )
    XDrawLine( display, bm.win, xorgc,
               slitxs, slitys, slitxe, slitye );
   slitxe= px;
   slitye= py;
   if ( XQueryPointer( display, bm.win, &windum, &windum,
                       &dummy, &dummy, &px, &py, &keys ) == True &&
        !( keys & Button2Mask ) ) { /* the button was released, so handle it */
    doslit( display_to_imagecol( slitxs ),
            display_to_imagerow( slitys ),
   ...
   if ( event.xbutton.button == Button3 ) {
    ...
    if ( mousemode == 0 ) { /* new center */
     if ( event.xbutton.state & ControlMask ) {
      ...
      if ( bppix == 16 )
       luttransoff=
           ( 65536 * ( event.xbutton.x - (int)bm.width / 2 ) ) / ( (int)bm.width / 2 );
      else
       luttransoff= ( 256 *
