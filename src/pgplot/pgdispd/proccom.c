...
int proccom( buf, len, retbuf, retbuflen ) unsigned short *buf; /* the buffer of commands and arguments */
...
int *retbuflen;                                                 /* the length of retbuf */
{
 ...
 if ( !len & savedshorts ) { /* an incomplete command was sent! */
  savedshorts= 0;
  free( (char *)olddata );
  return ( INCCOM );
 }
 if ( savedshorts ) { /* There's some data left over from the old command */
  usedolddata= 1;
#ifdef lint
  bothbuf= NULL;
  if ( !( malloc(
 ...
 while ( len-- > 0 ) { /* until there are no more commands to process */
  switch ( ntohs( *buf++ ) ) {
  ...
  case SHOW_LG_WIN: /* conceal/reveal line graphics */
   if ( !len ) {    /* save this data for the next time around */
    savedshorts= 1;
    --buf;
    break;
   }
  ...
  case LG_CURS: /* set and get cursor location */
   ...
   if ( pggcurs( &bufcont[0] ) ) { /* There's already an event there */
  ...
  case DRAW_LINE: /* draw a line */
   ...
   XDrawLine( display, lg.pixmap, linegc, minx, miny, maxx,
              maxy );
   if ( minx > maxx ) { /* swap for the XCopyArea */
    i= minx;
    minx= maxx;
    maxx= i;
   }
   if ( miny > maxy ) { /* swap for the XCopyArea */
    i= miny;
    miny= maxy;
    maxy= i;
   }
  ...
  case DRAW_POLY: /* draw a poly line */
   ...
   maxy= 0;
   while ( i-- > 0 ) {
    if ( (int)ntohs( *buf ) < minx )
     minx= ntohs( *buf );
    if ( (int)ntohs( *buf ) > maxx )
     maxx= ntohs( *buf );
    points[j].x= ntohs( *buf++ );
    if ( (int)ntohs( *buf ) < miny )
     miny= ntohs( *buf );
    if ( (int)ntohs( *buf ) > maxy )
     maxy= ntohs( *buf );
  ...
  case BM_GET_CURS: /* set and get cursor location */
   ...
   buflen= 4;
   if ( bmgcurs( &bufcont[0] ) ) { /* There's already an event there */
    returnbuf( &bufcont[0], 4, srcwin );
    buflen= 0;
   }
#endif
  ...
  case SET_BM_SIZE: /* set size of image */
   ...
   len-= 3;
   if ( minx= resizeim( minx, miny, maxx ) )
    return ( minx );
#endif
   break;
  case BM_WRITE:
   ...
   --len;
   if ( minx == 16 )
    savedshorts= writeimage16( &buf, &len );
   else
    savedshorts= writeimage( &buf, &len );
#endif
   break;
  case SET_BM_CSCALE:
#ifdef PGDISP
 ...
 if ( usedolddata )
  free( (char *)bothbuf );
 if ( !savedshorts && buflen ) { /* if there's no incomplete command and we need to send a message */
  for ( i= 0; i < buflen; ++i )
   retbuf[i]= bufcont[i];
  *retbuflen= buflen;
  buflen= 0;
