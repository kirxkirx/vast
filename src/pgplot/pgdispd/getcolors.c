...
int getcolors( vistype, visual, cmap, pix, maxcolors, mincolors, depth,
               maxdepth, mindepth ) int vistype; /* The type of visual to use */
...
int mindepth;                                    /* The minimum allowed visual depth */
{
 ...
 if ( vistype == UseDefaultCmap ) { /* This is the easy case */
  ...
  *depth= maxdepth;
  ...
  while ( *depth >= mindepth ) {
   if ( XMatchVisualInfo( display, screen, *depth, class,
                          &vinfo ) ) { /* we got a visual - can we allocate enough colors? */
    *visual= vinfo.visual;
    *cmap= XCreateColormap( display,
    ...
    if ( vistype == UseROVisual ) {
     if ( maxcolors > ( *visual )->map_entries )
      maxcolors=
          ( *visual )->map_entries;
     if ( maxcolors < mincolors ) { /* this shouldn't happen... */
      --( *depth );
      continue;
     }
     return ( maxcolors );
