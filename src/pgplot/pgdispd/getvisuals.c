...
int getvisuals() {
 ...
 if ( res.maxpcolors < res.maxcolors )
  i= res.maxcolors;
 else
  i= res.maxpcolors;
 i+= lg.colors;
 ...
 while ( lg.colors > 2 ) {
  ...
  for ( i= 0; i < 3; ++i ) {
   if ( i == 0 && ( bm.colors= getcolors( UseDefaultCmap, &bitvisual, &bitcmap, bm.pix, res.maxcolors + res.leavecolors, res.mincolors + res.leavecolors, &bitdepth, 8, 8 ) ) && ( lg.colors= getcolors( UseRWVisual, &linevisual, &linecmap, lg.pix, res.lgcolors + res.savecolors, res.lgcolors + res.savecolors, &linedepth, MAX_DEPTH, 1 ) ) || i == 1 && ( bm.colors= getcolors( UseRWVisual, &bitvisual, &bitcmap, bm.pix, res.maxpcolors + res.savecolors, res.minpcolors + res.savecolors, &bitdepth, 8, 8 ) ) && ( lg.colors= getcolors( UseDefaultCmap, &linevisual, &linecmap, lg.pix, res.lgcolors + res.leavecolors, res.lgcolors + res.leavecolors, &linedepth, MAX_DEPTH, 1 ) ) || i == 2 && ( bm.colors= getcolors( UseRWVisual, &bitvisual, &bitcmap, bm.pix, res.maxpcolors + res.savecolors, res.minpcolors + res.savecolors, &bitdepth, 8, 8 ) ) && ( lg.colors= getcolors( UseRWVisual, &linevisual, &linecmap, lg.pix, res.lgcolors + res.savecolors, res.lgcolors + res.savecolors, &linedepth, MAX_DEPTH, 1 ) ) )
    break;

   /* if we got here, we may have been able to allocate */
   /* some bitmap colors */
   if ( bm.colors )
    XFreeColors( display, bitcmap, bm.pix,
                 bm.colors, 0 );
  }

  if ( bm.colors && lg.colors ) { /* whew - something worked */
   if ( bitcmap == DefaultColormap( display, screen ) ) {
    if ( res.leavecolors > 0 )
     XFreeColors( display,
  ...
  /* 16 colors */
  if ( res.lgcolors > 16 )
   res.lgcolors= lg.colors= 16;
  else
   res.lgcolors= lg.colors= 2;
 }

 if ( linevisual != NULL && linevisual->class == GrayScale )
  lg.bw= 1;
 if ( bitvisual != NULL && bitvisual->class == GrayScale )
  bm.bw= 1;
 ...
 while ( res.lgcolors > 2 ) {
  if ( lg.colors= getcolors( UseROVisual, &linevisual, &linecmap,
                             lg.pix, res.lgcolors, res.lgcolors, &linedepth ) )
   break;
 ...
 if ( res.lgcolors <= 2 ) { /* Sorry, all we can do is BlackPixel & WhitePixel */
 ...
 if ( linevisual->class == StaticGray )
  lg.bw= 1;
 if ( bitvisual->class == GrayScale )
  bm.bw= 1;
