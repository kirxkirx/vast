...
int *selset;               /* whether or not the selection is owned by a user program */
{
 ...
 if ( buflen == -1 )
  buflen= ( XMaxRequestSize( display ) - 10 ) << 1;
 ...
 if ( acttype == XA_STRING ) {
  if ( proccom( buffer, (int)nitems >> 1, rbuf, rbuflen ) )
   return ( 0 );
 } else if ( acttype == incrtype ) {
  if ( !nitems ) { /* all done, get the selection back */
   ...
   *selset= 0;
   ...
   if ( *rbuflen )
    (void)fprintf( stderr, MSG_REPLYNOTSENT );
   *rbuflen= 0;
   XFlush( display );
  }
 } else
  (void)fprintf( stderr, MSG_BADDATATYPE );
 XFree( (char *)buffer );
 ...
 if ( ntohs( *rbuf ) != LG_CURS && ntohs( *rbuf ) != BM_GET_CURS ) { /* cursor information isn't here yet */
