...
int getcurs( buf, curlist, listend ) short *buf;
    ...
    XEvent event;
{
 ...
 if ( event.type == ButtonPress )
  getbuttonval( event.xbutton.button, &val );
 else if ( !getkeyval( event, &val ) )
  return ( 1 );
 ...
 if ( lastlg != NULL ) {
  ...
  if ( malloc( sizeof( struct curpos ) ) == NULL )
#else
  if ( ( lgcurses= (struct curpos *)malloc( sizeof( struct curpos ) ) ) == NULL )
#endif
   return ( 1 );
  lastlg= lgcurses;
 }
    ...
    XEvent event;
{
 ...
 if ( lastbm != NULL ) {
  ...
  if ( malloc( sizeof( struct curpos ) ) == NULL )
#else
  if ( ( bmcurses= (struct curpos *)malloc( sizeof( struct curpos ) ) ) == NULL )
#endif
   return ( 1 );
  lastbm= bmcurses;
 }
...
void getbuttonval( button, val ) unsigned int button;
short *val;
{
 switch ( button ) {
 case Button1:
...
void bmscurs( x, y ) int x, y;
{
 ...
 if ( ( x + 1 < bmx || x - 1 > bmx || y - 1 > bmy || y + 1 < bmy ) &&
      imagecol_to_display( x ) >= 0 && imagecol_to_display( x ) < bm.width && imagerow_to_display( y ) >= 0 &&
