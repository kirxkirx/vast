...
static int shared_destroy_entry( int idx ) /* unconditionally destroy sema & shseg and clear entry */
{
 ...
 if ( SHARED_INVALID != shared_gt[idx].handle )
  r2= shmctl( shared_gt[idx].handle, IPC_RMID, 0 ); /* destroy shared memory segment */
 if ( SHARED_OK == r )
 ...
 if ( NULL != shared_gt ) /* detach global index table */
 {
  ...
  if ( -1 != fcntl( shared_fd, F_SETLK, &flk ) ) {
   ...
   if ( 0 == segmentspresent )                       /* if there are no segs ... */
    ...
    {
     if ( ds.shm_nattch <= 1 )
      oktodelete= 1; /* if only one (we), then it is safe (but see text 4 lines later) to unlink */
    }
  }
...
int shared_init( int debug_msgs ) /* initialize shared memory stuff, you have to call this routine once */
{
 ...
 if ( NULL != ( p= getenv( SHARED_ENV_KEYBASE ) ) )
  shared_kbase= atoi( p );
 if ( 0 == shared_kbase )
  shared_kbase= SHARED_KEYBASE;
 if ( shared_debug )
  printf( " keybase=%d", shared_kbase );

 shared_maxseg= 0;
 if ( NULL != ( p= getenv( SHARED_ENV_MAXSEG ) ) )
  shared_maxseg= atoi( p );
 if ( 0 == shared_maxseg )
  shared_maxseg= SHARED_MAXSEG;
 if ( shared_debug )
  printf( " maxseg=%d", shared_maxseg );
 ...
 if ( SHARED_INVALID == shared_gt_h ) /* global table not attached, try to create it in shared memory */
 {
  ...
  if ( SHARED_INVALID == shared_gt_h )                                                                                   /* if failed, try to open as a slave */
  {
   ...
   if ( ( (SHARED_GTAB *)SHARED_INVALID ) == shared_gt )
   ...
   if ( ( (SHARED_GTAB *)SHARED_INVALID ) == shared_gt )
    return ( SHARED_IPCERR );
   for ( i= 0; i < shared_maxseg; i++ )
    shared_clear_entry( i ); /* since we are master, init data */
   if ( shared_debug )
    printf( "master" );
  }
 }
 ...
 if ( NULL == shared_lt ) /* initialize local table */
 {
  ...
  if ( NULL == ( shared_lt= (SHARED_LTAB *)malloc( shared_maxseg * sizeof( SHARED_LTAB ) ) ) )
   return ( SHARED_NOMEM );
  for ( i= 0; i < shared_maxseg; i++ ) {
...
int shared_recover( int id ) /* try to recover dormant segments after applic crash */
{
 ...
 for ( i= 0; i < shared_maxseg; i++ ) {
  ...
  if ( shared_mux( i, SHARED_NOWAIT | SHARED_RDWRITE ) )
  ...
  if ( ( shared_gt[i].nprocdebug > r2 ) || ( 0 == r2 ) ) {
   if ( shared_debug )
    printf( "Bogus handle=%d nproc=%d sema=%d:", i, shared_gt[i].nprocdebug, r2 );
...
static int shared_get_free_entry( int newhandle ) /* get newhandle, or -1, entry is set rw locked */
{
 ...
 if ( shared_mux( newhandle, SHARED_NOWAIT | SHARED_RDWRITE ) )
 ...
 if ( shared_debug )
  printf( "[free_entry - ERROR - entry unusable]" );
 return ( -1 ); /* table full */
}
...
int shared_malloc( long size, int mode, int newhandle ) /* return idx or SHARED_INVALID */
{
 ...
 if ( shared_debug )
  printf( "malloc (size = %ld, mode = %d):", size, mode );
 if ( size < 0 )
  return ( SHARED_INVALID );
 if ( -1 == ( idx= shared_get_free_entry( newhandle ) ) )
  return ( SHARED_INVALID );
 if ( shared_debug )
  printf( " idx=%d", idx );
 for ( i= 0;; i++ ) {
  ...
  if ( shared_debug )
   printf( " sem=%d", shared_gt[idx].sem );
...
int shared_attach( int idx ) {
 ...
 if ( SHARED_OK != ( r= shared_mux( idx, SHARED_RDWRITE | SHARED_WAIT ) ) )
  return ( r );
...
static int shared_map( int idx ) /* map all tables for given idx, check for validity */
{
 ...
 if ( SHARED_INVALID == shared_gt[idx].key )
  return ( SHARED_BADARG );
 if ( SHARED_INVALID == ( h= shmget( shared_gt[idx].key, 1, shared_create_mode ) ) )
  return ( SHARED_BADARG );
 if ( ( (BLKHEAD *)SHARED_INVALID ) == ( bp= (BLKHEAD *)shmat( h, 0, 0 ) ) )
  return ( SHARED_BADARG );
...
static int shared_validate( int idx, int mode ) /* use intrnally inside crit.sect !!! */
{
 ...
 if ( SHARED_OK != ( r= shared_mux( idx, mode ) ) )
...
SHARED_P shared_realloc( int idx, long newsize ) /* realloc shared memory segment */
{
 ...
 for ( i= 0;; i++ ) {
  if ( i >= shared_range )
   return ( NULL ); /* table full, signal error & exit */
  ...
  *bp= *( shared_lt[idx].p ); /* copy header, then data */
  ...
  if ( shmctl( shared_gt[idx].handle, IPC_RMID, 0 ) )
   if ( SHARED_OK == r )
...
int shared_free( int idx ) /* detach segment, if last process & !PERSIST, destroy segment */
{
 ...
 if ( SHARED_OK != ( r= shared_validate( idx, SHARED_RDWRITE | SHARED_WAIT ) ) )
  return ( r );
...
SHARED_P shared_lock( int idx, int mode ) /* lock given segment for exclusive access */
{
 ...
 if ( shared_mux( idx, mode ) )
  return ( NULL );                /* idx checked by shared_mux */
...
int shared_unlock( int idx ) /* unlock given segment, assumes seg is locked !! */
{
 ...
 if ( SHARED_OK != ( r= shared_check_locked_index( idx ) ) )
  return ( r );
...
int smem_open( char *filename, int rwmode, int *driverhandle ) {
 ...
 if ( SHARED_OK != ( r= shared_attach( h ) ) )
  return ( r );
...
int smem_close( int driverhandle ) {
 ...
 if ( SHARED_OK != ( r= shared_unlock( driverhandle ) ) )
  return ( r );
...
int smem_remove( char *filename ) {
...

 {
  ...
  {
   if ( SHARED_OK != ( r= shared_unlock( h ) ) )
    return ( r ); /* yes, so relock in RW */
   if ( NULL == shared_lock( h, SHARED_RDWRITE ) )
    return ( SHARED_BADARG );
  }
...
int smem_size( int driverhandle, LONGLONG *size ) {
 if ( NULL == size )
  return ( SHARED_NULPTR );
 if ( shared_check_locked_index( driverhandle ) )
  return ( SHARED_INVALID );
...
int smem_write( int driverhandle, void *buffer, long nbytes ) {
 ...
 if ( nbytes < 0 )
  return ( SHARED_BADARG );
 if ( (unsigned long)( shared_lt[driverhandle].seekpos + nbytes ) > (unsigned long)( shared_gt[driverhandle].size - sizeof( DAL_SHM_SEGHEAD ) ) ) { /* need to realloc shmem */
