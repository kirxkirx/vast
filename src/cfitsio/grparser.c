...
int ngp_get_extver( char *extname, int *version ) {
 ...
 if ( ( NULL == extname ) || ( NULL == version ) )
  return ( NGP_BAD_ARG );
 if ( ( NULL == ngp_extver_tab ) && ( ngp_extver_tab_size > 0 ) )
  return ( NGP_BAD_ARG );
 if ( ( NULL != ngp_extver_tab ) && ( ngp_extver_tab_size <= 0 ) )
  return ( NGP_BAD_ARG );
...
int ngp_set_extver( char *extname, int version ) {
 ...
 for ( i= 0; i < ngp_extver_tab_size; i++ ) {
  if ( 0 == strcmp( extname, ngp_extver_tab[i].extname ) ) {
   if ( version > ngp_extver_tab[i].version )
    ngp_extver_tab[i].version= version;
   return ( NGP_OK );
  }
 }
...
int ngp_delete_extver_tab( void ) {
 ...
 if ( ( NULL != ngp_extver_tab ) && ( ngp_extver_tab_size <= 0 ) )
  return ( NGP_BAD_ARG );
 if ( ( NULL == ngp_extver_tab ) && ( 0 == ngp_extver_tab_size ) )
  return ( NGP_OK );
...
int ngp_extract_tokens( NGP_RAW_LINE *cl ) {
 ...
 {
  if ( ( 0 == *p ) || ( '\n' == *p ) ) { /* if line has only blanks -> write blank keyword */
 ...
 if ( ( !fits_strcasecmp( "HISTORY", cl->name ) ) || ( !fits_strcasecmp( "COMMENT", cl->name ) ) || ( !fits_strcasecmp( "CONTINUE", cl->name ) ) ) {
  cl->comment= p;
  for ( s= cl->comment;; s++ ) /* filter out any EOS characters in comment */
  {
   if ( '\n' == *s )
 ...
 for ( ;; p++ ) {
  ...
  if ( cl_flags & NGP_FOUND_EQUAL_SIGN )
   break;
  if ( '=' != *p )
 ...
 if ( '\'' == *p ) /* we have found string within quotes */
 {
  ...
  for ( ;; ) /* analyze it */
  {
   ...
   if ( '\'' == *p ) /* we have found doublequote */
   {
    ...
    if ( '\'' == p[1] )
     p++; /* doublequote is inside string, convert "" -> " */
   }
...
int ngp_include_file( char *fname ) /* try to open include file */
{
 ...
 if ( NULL == ( ngp_fp[ngp_inclevel]= fopen( fname, "r" ) ) ) { /* if simple open failed .. */
...
int ngp_read_line( int ignore_blank_lines ) {
 ...
 for ( ;; ) {
  ...
  case NGP_OK:
   if ( ngp_curline.flags & NGP_LINE_REREAD )
    return ( r );
   break;
  default:
   return ( r );
  ...
  case 0:
   if ( 0 == ignore_blank_lines )
    break; /* ignore empty lines if told so */
  case '#':
  ...
  {
   if ( NGP_TOKEN_UNKNOWN == ngp_tkdef[k].code )
    break;
   if ( 0 == strcmp( ngp_curline.name, ngp_tkdef[k].name ) )
    break;
  }
  ...
  {
   if ( NGP_OK != ( r= ngp_include_file( ngp_curline.value ) ) )
    return ( r );
  ...
  if ( NULL != ngp_curline.value ) /* if no value given signal it */
  {
   ...
   if ( NGP_TTYPE_RAW == ngp_curline.type )
    ngp_linkey.type= NGP_TTYPE_RAW;
   else
    ngp_linkey.type= NGP_TTYPE_NULL;
  }
...
int ngp_keyword_is_write( NGP_TOKEN *ngp_tok ) {
 ...
 for ( j= 0;; j++ ) /* first check non indexed */
 {
  if ( NULL == nmni[j] )
   break;
  if ( 0 == strcmp( nmni[j], ngp_tok->name ) )
   return ( NGP_BAD_ARG );
 }
 ...
 for ( j= 0;; j++ ) /* now check indexed */
 {
  ...
  if ( ( l < 1 ) || ( l > 5 ) )
   continue;
  if ( 0 == strncmp( nm[j], ngp_tok->name, l ) )
   break;
 }

 if ( ( ngp_tok->name[l] < '1' ) || ( ngp_tok->name[l] > '9' ) )
  return ( NGP_OK );
 spc= 0;
 for ( i= l + 1; i < 8; i++ ) {
  if ( spc ) {
   ...
   if ( ( ngp_tok->name[i] >= '0' ) && ( ngp_tok->name[i] <= '9' ) )
    continue;
   if ( ' ' == ngp_tok->name[i] ) {
    spc= 1;
    continue;
...
int ngp_hdu_insert_token( NGP_HDU *ngph, NGP_TOKEN *newtok ) {
 ...
 if ( NGP_TTYPE_STRING == newtok->type ) {
  if ( NULL != newtok->value.s ) {
   ...
   if ( NULL == ngph->tok[ngph->tokcnt].value.s )
    return ( NGP_NO_MEMORY );
...
int ngp_read_xtension( fitsfile *ff, int parent_hn, int simple_mode ) {
 ...
 if ( NGP_OK != ( r= ngp_hdu_init( &ngph ) ) )
  return ( r );

 if ( NGP_OK != ( r= ngp_read_line( 0 ) ) )
  return ( r ); /* EOF always means error here */
 switch ( NGP_XTENSION_SIMPLE & simple_mode ) {
 case 0:
  if ( NGP_TOKEN_XTENSION != ngp_keyidx )
   return ( NGP_TOKEN_NOT_EXPECT );
  break;
 default:
  if ( NGP_TOKEN_SIMPLE != ngp_keyidx )
   return ( NGP_TOKEN_NOT_EXPECT );
  break;
 }
 ...
 if ( NGP_OK == r ) { /* we should scan keywords, and calculate HDU's */
                      /* structure ourselves .... */

  ngph_node_type= NGP_NODE_INVALID; /* init variables */
  ngph_bitpix= 0;
  ngph_extname= NULL;
  for ( i= 0; i < NGP_MAX_ARRAY_DIM; i++ )
   ngph_size[i]= 0;
  ngph_dim= 0;
  ...
  case NGP_NODE_IMAGE:
   if ( NGP_XTENSION_FIRST == ( ( NGP_XTENSION_FIRST | NGP_XTENSION_SIMPLE ) & simple_mode ) ) { /* if caller signals that this is 1st HDU in file */
  ...
  case NGP_NODE_BTABLE:
   ...
   fits_create_tbl( ff, ( ( NGP_NODE_ATABLE == ngph_node_type ) ? ASCII_TBL : BINARY_TBL ),
   ...
   if ( ngph_size[1] > 0 )
    fits_insert_rows( ff, 0, ngph_size[1], &r );
   break;
...
int ngp_read_group( fitsfile *ff, char *grpname, int parent_hn ) {
 ...
 r= NGP_OK;
 if ( NGP_OK != ( r= fits_create_group( ff, grpname, GT_ID_ALL_URI, &r ) ) )
  return ( r );
 fits_get_hdu_num( ff, &my_hn );
 if ( parent_hn > 0 ) {
  fits_movabs_hdu( ff, parent_hn, &tmp0, &r ); /* link us to parent */
...
int fits_execute_template( fitsfile *ff, char *ngp_template, int *status ) {
 ...
 if ( my_hn <= 1 )               /* check whether we really need to create PHDU */
 {
  ...
  if ( NGP_OK != *status )
   return ( *status ); /* error here means file is corrupted */
  if ( keys_exist > 0 )
   ...
   *status= NGP_OK;
   ...
   if ( NGP_OK == *status )
    *status= ngp_set_extver( used_name, used_ver );
  }
