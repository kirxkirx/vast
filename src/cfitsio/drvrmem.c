...
*/
{
 ...
 if ( memTable[handle].mem_realloc ) { /* explicit LONGLONG->size_t cast */
...
*/
  ...
  if ( sizeof( size_t ) > 4 && filesize > 10000 ) {
   ...
   while ( llsize < (LONGLONG)filesize )
    llsize+= 4294967296;
...
*/
{
 ...
 if ( ( size_t )( memTable[hdl].currentpos + nbytes ) >
  ...
  newsize= maxvalue( ( size_t )( ( ( memTable[hdl].currentpos + nbytes - 1 ) / 2880 ) + 1 ) * 2880,
