 ...
 *exact= TRUE;
 ...
 if ( !casesen ) { /* convert both strings to uppercase before comparison */
  ffupch( temp );
  ffupch( col );
 }
...
*/
{
 ...
 while ( ( ( fptr->Fptr )->curhdu ) + 1 != hdunum ) /* at the correct HDU? */
 {
  ...
  if ( ( fptr->Fptr )->headstart[moveto] < ( fptr->Fptr )->logfilesize ) {
   ...
   {
    if ( ffgext( fptr, moveto, exttype, status ) > 0 ) { /* failed to get the requested extension */
...
*/
{
 ...
 if ( ffmbyt( fptr, ( fptr->Fptr )->headstart[hdunum], REPORT_EOF, status ) <= 0 ) {
  ...
  if ( ffrhdu( fptr, exttype, status ) > 0 ) { /* failed to get the new HDU, so restore previous values */
...
*/
{
 ...
 if ( *card == 'Z' ) {
  ...
  if ( *card1 == 'C' ) {
   ...
   if ( *( card + 2 ) >= '0' && *( card + 2 ) <= '9' )
    return ( TYP_WCS_KEY );
  } else if ( *card1 >= '0' && *card1 <= '9' ) { /* 2 digits at beginning of keyword */
