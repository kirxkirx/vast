...
*/
{
 ...
 if ( longbitpix != obitpix ) { /* update BITPIX value */
  ffmkyj( fptr, "BITPIX", longbitpix, comment, status );
 }

 if ( naxis != onaxis ) { /* update NAXIS value */
  longval= naxis;
...
*/
{
 ...
 for ( ii= 1; ii < nrows; nextrow++, nextrowpos+= naxis1 ) {
  if ( nextrow < rownum[ii] ) { /* keep this row, so copy it to the new position */
   ...
   insertpos+= naxis1;
  } else { /* skip over this row since it is in the list */
   ii++;
  }
 }
...
*/
{
 ...
 if ( create_col ) /* insert new column in output table? */
 {
  ...
  if ( ( infptr->Fptr == outfptr->Fptr ) && ( infptr->HDUposition == outfptr->HDUposition ) && ( colnum <= incol ) ) {
 ...
 if ( typecode == TLOGICAL ) {
  ...
  if ( !lvalues ) {
   ffpmsg( "malloc failed to get memory for logicals (ffcpcl)" );
  ...
  if ( !fvalues ) {
   ffpmsg( "malloc failed to get memory for complex (ffcpcl)" );
   return ( *status= ARRAY_TOO_BIG );
  }
  fnull= 0.;
  ...
  if ( !dvalues ) {
   ffpmsg( "malloc failed to get memory for dbl complex (ffcpcl)" );
   return ( *status= ARRAY_TOO_BIG );
  }
  dnull= 0.;
 } else /* numerical datatype; read them all as doubles */
 {
  ...
  if ( !dvalues ) {
   ffpmsg( "malloc failed to get memory for doubles (ffcpcl)" );
   return ( *status= ARRAY_TOO_BIG );
  }
 ...
 while ( ntodo ) /* iterate through the table */
 {
  ...
  else if ( typecode == TCOMPLEX ) { /* doesn't support writing nulls */
   ffpclc( outfptr, colnum, firstrow, firstelem, ntodo,
           fvalues, status );
  }

  else if ( typecode == TDBLCOMPLEX ) { /* doesn't support writing nulls */
