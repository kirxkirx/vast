...
int imcomp_convert_tile_tshort(
 ...
 if ( ( outfptr->Fptr )->compress_type == RICE_1 || ( outfptr->Fptr )->compress_type == GZIP_1 || ( outfptr->Fptr )->compress_type == GZIP_2 || ( outfptr->Fptr )->compress_type == BZIP2_1 ) {
...
*/
 ...
 for ( i4= 0, it4= 0; i4 <= imglpix[4] - imgfpix[4]; i4++, it4++ ) {
  ...
  while ( ndim > 4 && ( tfpixel[4] + tilefpix[4] - fpixel[4] + it4 ) % labs( inc[4] ) != 0 )
   it4++;
  ...
  for ( i3= 0, it3= 0; i3 <= imglpix[3] - imgfpix[3]; i3++, it3++ ) {
   /* increment plane if it falls in the cracks of the subsampled image */
   while ( ndim > 3 && ( tfpixel[3] + tilefpix[3] - fpixel[3] + it3 ) % labs( inc[3] ) != 0 )
    it3++;
   ...
   for ( i2= 0, it2= 0; i2 <= imglpix[2] - imgfpix[2]; i2++, it2++ ) {
    ...
    while ( ndim > 2 && ( tfpixel[2] + tilefpix[2] - fpixel[2] + it2 ) % labs( inc[2] ) != 0 )
     it2++;
    ...
    for ( i1= 0, it1= 0; i1 <= imglpix[1] - imgfpix[1]; i1++, it1++ ) {
     ...
     while ( ndim > 1 && ( tfpixel[1] + tilefpix[1] - fpixel[1] + it1 ) % labs( inc[1] ) != 0 )
      it1++;
