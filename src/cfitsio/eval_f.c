...
/*--------------------------------------------------------------------------*/
{
 ...
 if ( *status ) {
  ...
  if ( infptr == outfptr ) { /* Skip initial good rows if input==output file */
   while ( ( (char *)Info.dataPtr )[inloc - 1] )
    inloc++;
   outloc= inloc;
  } else {
...
/*--------------------------------------------------------------------------*/
{
 ...
 if ( expr[0] == '@' ) {
  if ( ffimport_file( expr + 1, &gParse.expr, status ) )
   return ( *status );
...
/*--------------------------------------------------------------------------*/
{
 ...
 if ( gParse.nCols > 0 ) {
  ...
  for ( col= 0; col < gParse.nCols; col++ ) {
   if ( gParse.varData[col].undef == NULL )
    continue;
  ...
  if ( userInfo->dataPtr == NULL ) {
   ...
   if ( gParse.hdutype == IMAGE_HDU ) {
    ...
    if ( status == BAD_INTKEY ) {
     ...
     case TSHORT:
      jnull= (LONGLONG)SHRT_MIN;
      break;
     case TINT:
      jnull= (LONGLONG)INT_MIN;
      break;
     case TLONG:
      jnull= (LONGLONG)LONG_MIN;
      break;
     }
    }
   }
...
*/
  ...
  case BOOLEAN:
   resDataSize= sizeof( char );
   break;
  case LONG:
   resDataSize= sizeof( long );
   break;
  case DOUBLE:
   resDataSize= sizeof( double );
   break;
  }
 }
...
*/
 ...
 if ( userInfo->dataPtr == NULL ) {
   ...
   *(char *)Null= 'U';
   break;
  case TBYTE:
   *(char *)Null= (char)jnull;
   break;
  case TSHORT:
   *(short *)Null= (short)jnull;
   break;
  case TINT:
   *(int *)Null= (int)jnull;
   break;
  case TLONG:
   *(long *)Null= (long)jnull;
   break;
  case TLONGLONG:
   *(LONGLONG *)Null= (LONGLONG)jnull;
   break;
  case TFLOAT:
   *(float *)Null= FLOATNULLVALUE;
   break;
  case TDOUBLE:
   *(double *)Null= DOUBLENULLVALUE;
   break;
  case TSTRING:
   ( *(char **)Null )[0]= '\1';
   ( *(char **)Null )[1]= '\0';
 ...
 while ( remain ) {
  ...
  case DOUBLE:
   if ( constant ) {
    ...
    if ( repeat == result->value.nelem ) {
     ...
     for ( kk= 0; kk < ntodo; kk++ ) {
      ffcvtn( gParse.datatype,
              (char *)result->value.data.ptr + kk * result->value.nelem * resDataSize,
              (char *)result->value.undef + kk * result->value.nelem,
  ...
  if ( result->type == BITSTR && userInfo->datatype == TBYTE )
   Data= (char *)Data + datasize * ( ( result->value.nelem + 7 ) / 8 ) * ntodo;
...
/*---------------------------------------------------------------------------*/
{
 ...
 for ( row= 1; row <= naxis2; row++ ) {
  ...
  parNo= gParse.nCols;
  while ( parNo-- )
   if ( !fits_strcasecmp( parName, gParse.varData[parNo].name ) )
    break;
...
static int find_column( char *colName, void *itslval ) {
 ...
 if ( gParse.hdutype == IMAGE_HDU ) {
  ...
  if ( allocateCol( col_cnt, &gParse.status ) )
   return pERROR;
  ...
  else if ( fits_get_colnum( fptr, CASEINSEN, colName, &colnum, &status ) ) {
...
static int allocateCol( int nCol, int *status ) {
 if ( ( nCol % 25 ) == 0 ) {
  ...
  if ( gParse.colData == NULL || gParse.varData == NULL ) {
   if ( gParse.colData )
    free( gParse.colData );
   if ( gParse.varData )
    free( gParse.varData );
