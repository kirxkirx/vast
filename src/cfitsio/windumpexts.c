...
#endif
    if ( ( stricmp( s, "DllEntryPoint" ) != 0 ) && ( stricmp( s, "DllMain" ) != 0 ) ) {
     fprintf( fout, "\t%s\n", s );
    }
   }
  }
...
void DumpObjFile( PIMAGE_FILE_HEADER pImageFileHeader, FILE *fout, int full ) {
 ...
 PCOFFSymbolTable= ( PIMAGE_SYMBOL )( (DWORD)pImageFileHeader + pImageFileHeader->PointerToSymbolTable );
...
void DumpFile( LPSTR filename, FILE *fout, int full ) {
 ...
 else if ( ( dosHeader->e_magic == e_magic_number ) && ( dosHeader->e_sp == 0 ) ) {
  /*
