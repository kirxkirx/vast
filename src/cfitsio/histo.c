 ...
 *wtname= '\0';
 ...
 if ( *ptr == '@' ) {
  if ( ffimport_file( ptr + 1, &file_expr, status ) )
   return ( *status );
  ptr= file_expr;
  while ( *ptr == ' ' )
...
int ffhist( fitsfile **fptr,             /* IO - pointer to table with X and Y cols;    */
 ...
 for ( ii= 0; ii < naxis; ii++ ) {
  ...
  /* get the column number in the table */
  if ( ffgcno( *fptr, CASEINSEN, colname[ii], histData.hcolnum + ii, status ) > 0 ) {
 ...
 for ( ii= 0; ii < histData.haxis; ii++ ) {
  ...
  if ( tstatus ) { /* just use column name as the type */
   tstatus= 0;
  ...
  ffgky( *fptr, TSTRING, keyname, svalue, NULL, &tstatus );
  if ( tstatus ) { /* use the column units */
   tstatus= 0;
...
int fits_rebin_wcsd(
 ...
 for ( ii= 0; ii < naxis; ii++ ) {
  ...
  if ( !tstatus ) {
   ...
   fits_modify_key_dbl( fptr, keyname, dvalue, -14, NULL, &tstatus );
  } else { /* no CDELTn keyword, so look for a CDij keywords */
   reset= 0;
