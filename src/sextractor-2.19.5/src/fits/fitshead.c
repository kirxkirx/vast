...

{
 ...
 if ( fitsread( tab->headbuf, "BITPIX  ", &tab->bitpix, H_INT, T_LONG ) == RETURN_ERROR )
  error( EXIT_FAILURE, "*Error*: Corrupted FITS header in ", filename );

 tab->bytepix= tab->bitpix > 0 ? ( tab->bitpix / 8 ) : ( -tab->bitpix / 8 );

 if ( fitsread( tab->headbuf, "NAXIS   ", &tab->naxis, H_INT, T_LONG ) == RETURN_ERROR )
  error( EXIT_FAILURE, "*Error*: Corrupted FITS header in ", filename );
 ...
 if ( tab->naxis > 0 ) {
  ...
  for ( i= 0; i < tab->naxis && i < 999; i++ ) {
   ...
   if ( fitsread( tab->headbuf, key, &tab->naxisn[i], H_INT, T_LONG ) == RETURN_ERROR )
 ...
 tab->blankflag=
     ( fitsread( tab->headbuf, "BLANK   ", &tab->blank, H_INT, T_LONG ) == RETURN_OK ) ? 1 : 0;
...

{
 ...
 if ( ( tab->naxis != 2 ) || ( tab->bitpix != 8 ) || ( tab->tfields == 0 ) || strncmp( tab->xtension, "BINTABLE", 8 ) )
  return RETURN_ERROR;
 ...
 for ( i= 0; i < nfields; i++ ) {
  ...
  if ( fitsread( tab->headbuf, strk, key->name, H_STRING, T_STRING ) != RETURN_OK ) {
   error( EXIT_FAILURE,
...

{
 ...
 if ( ( fitsfind( tab->headbuf, keyword ) == RETURN_ERROR || findkey( keyword, (char *)histokeys, 12 ) != RETURN_ERROR ) && ( fitsfind( tab->headbuf, "END     " ) + 1 ) * 80 >= tab->headnblock * FBSIZE ) {
  tab->headnblock++;
