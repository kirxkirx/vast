 ...
 if ( ( tab= name_to_tab( cat, tabname, 0 ) ) ) {
  ...
  for ( i= 0, key= tab->key; i < tab->nkey; i++, key= key->nextkey ) {
   ...
   case T_BYTE:
    ...
    break;
   case T_SHORT:
    fprintf( stream, "Short Int" );
    break;
   case T_LONG:
    fprintf( stream, "Long Int" );
    break;
   case T_LONGLONG:
    fprintf( stream, "Long Long Int" );
    break;
   case T_FLOAT:
...

{
 ...
 if ( ( outtab= name_to_tab( cat, tab->extname, 0 ) ) ) {
  if ( ( outtab->naxis != 2 ) || ( outtab->bitpix != 8 ) || strcmp( outtab->xtension, tab->xtension ) || ( outtab->tfields != tab->tfields ) || ( outtab->naxisn[0] != tab->naxisn[0] ) )
   return RETURN_ERROR;
...

{
 ...
 if ( *tabname && ( outtab= name_to_tab( catout, tabname, 0 ) ) ) {
  if ( ( outtab->naxis != 2 ) || ( outtab->bitpix != 8 ) || strcmp( outtab->xtension, tabin->xtension ) || ( outtab->tfields != tabin->tfields ) || ( outtab->naxisn[0] != tabin->naxisn[0] ) )
   return RETURN_ERROR;
  prevtab= outtab;
...

{
 ...
 if ( ( tab->naxis != 2 ) || ( tab->bitpix != 8 ) || strncmp( tab->xtension, "BINTABLE", 8 ) )
  return RETURN_ERROR;
