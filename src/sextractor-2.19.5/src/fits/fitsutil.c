 ...
 *( (char *)ptr )= 0;
 ...
 /*Handle comments*/
 if ( (int)fitsline[8] != '=' ) {
  if ( strncmp( keyword, "COMMENT ", 8 ) && strncmp( keyword, "HISTORY ", 8 ) && strncmp( keyword, "HIERARCH", 8 ) && strncmp( keyword, "        ", 8 ) )
   return RETURN_ERROR;
  fptr= fitsline + 9;
  lastspace= NULL;
  for ( i= 71; i-- && ( c= *( fptr++ ) ); )
...

{
 ...
 if ( findkey( keyword, (char *)histokeys, 12 ) != RETURN_ERROR || ( pos= fitsfind( fitsbuf, keyword ) ) < 0 )
  return RETURN_ERROR;
 posoff= 10;
 fitsbuf+= 80 * pos;
 switch ( htype ) {
 ...
 case H_FLOAT:
  sprintf( str, "    %16.8f", ( ttype == T_DOUBLE ) ? *(double *)ptr : *(float *)ptr );
  break;

 case H_EXPO:
  sprintf( str, " %19.12E", ( ttype == T_DOUBLE ) ? *(double *)ptr : *(float *)ptr );
  break;

 case H_BOOL:
  if ( ( ttype == T_SHORT ) ? *(short *)ptr : ( ( ttype == T_BYTE ) ? *(BYTE *)ptr : *(LONG *)ptr ) )
   sprintf( str, "                   T" );
  else
   sprintf( str, "                   F" );
  break;
 ...
 case H_COMMENT:
  sprintf( str, "%-70s", (char *)ptr );
  posoff= 9;
  break;

  /* Special case of ``half-comments'' */
 case H_HCOMMENT:
  sprintf( str, " / %-47s", (char *)ptr );
  posoff= 30;
  break;
