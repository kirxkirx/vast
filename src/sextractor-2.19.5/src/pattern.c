...
void pattern_create( patternstruct *pattern, profitstruct *profit ) {
 ...
 if ( bt > PATTERN_BTMAX ) {
  ...
  scale= omwb * scale + wb * fabs( *profit->paramlist[PARAM_SPHEROID_REFF] / profit->pixstep ) * 1.5;
  aspect= omwb * aspect + wb * fabs( *profit->paramlist[PARAM_SPHEROID_ASPECT] );
  posangle/= DEG;
  dposangle= fmod_m90_p90( *profit->paramlist[PARAM_SPHEROID_POSANG] ) - posangle;
  if ( dposangle > 90.0 )
   dposangle-= 180.0;
 ...
 if ( r2max > ( dval= margin2 * x1cout * x1cout * det * det / ( cd12 * cd12 + cd22 * cd22 ) ) )
  r2max= dval;
 if ( r2max > ( dval= margin2 * x2cout * x2cout * det * det / ( cd21 * cd21 + cd11 * cd11 ) ) )
  r2max= dval;
    ...
    if ( !f ) {
     for ( i= npix; i--; pmodpix++ ) {
      ...
      if ( r2 > rl2 && r2 < rh2 ) {
       r= sqrt( r2 );
       dval= ( r < r0 ) ? ( r - rl ) / ( r0 - rl ) : ( rh - r ) / ( rh - r0 );
       *( pix++ )= (float)( dval= ( dval < 0.5 ) ? 2.0 * dval * dval : 1.0 - 2.0 * ( 1.0 - dval ) * ( 1.0 - dval ) );
       norm+= dval * dval;
      } else
       *( pix++ )= 0.0;
     }
 ...
 case PATTERN_POLARSHAPELETS:
  ...
  for ( n= 0; n <= nmax; n++ ) {
   for ( m= n % 2; m <= n; m+= 2 ) {
    ...
    norm= 0.0;
    for ( i= npix; i--; fr2t++ ) {
     *( pix++ )= (float)( dval= fac * pow( *fr2t, dm / 2.0 ) * psf_laguerre( *fr2t, hnmm, m ) * *( fexpr2t++ ) * cos( dm * *( fthetat++ ) ) );
     norm+= dval * dval;
    }
    pix-= npix;
    pnorm= norm * sbd * sbd;
    ...
    *( normt++ )= pnorm > 1.0 / BIG ? norm / sqrt( pnorm ) : 0.0;
    ...
    if ( m != 0 ) {
     ...
     norm= 0.0;
     for ( i= npix; i--; fr2t++ ) {
      *( pix++ )= (float)( dval= fac * pow( *fr2t, dm / 2.0 ) * psf_laguerre( *fr2t, hnmm, m ) * *( fexpr2t++ ) * sin( dm * *( fthetat++ ) ) );
      norm+= dval * dval;
     }
     pix-= npix;
     pnorm= norm * sbd * sbd;
