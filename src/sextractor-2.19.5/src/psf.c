...
psfstruct *psf_load( char *filename, int ext ) {
 ...
 if ( fitsread( head, "POLNAXIS", &ndim, H_INT, T_LONG ) == RETURN_OK && ndim ) {
  ...
  for ( i= 0; i < ndim; i++ ) {
   ...
   if ( fitsread( head, gstr, &psf->contextoffset[i], H_EXPO, T_DOUBLE ) != RETURN_OK )
    goto headerror;
   sprintf( gstr, "POLSCAL%1d", i + 1 );
   if ( fitsread( head, gstr, &psf->contextscale[i], H_EXPO, T_DOUBLE ) != RETURN_OK )
    goto headerror;
  }
 ...
 if ( fitsread( head, "PSF_SAMP", &psf->pixstep, H_FLOAT, T_FLOAT ) != RETURN_OK || psf->pixstep <= 0.0 )
  psf->pixstep= 1.0;
...
void psf_fit( psfstruct *psf, picstruct *field, picstruct *wfield,
 ...
 if ( prefs.check[CHECK_SUBPSFPROTOS] || prefs.check[CHECK_PSFPROTOS] || prefs.check[CHECK_SUBPCPROTOS] || prefs.check[CHECK_PCPROTOS] || prefs.check[CHECK_PCOPROTOS] ) {
 ...
 if ( wfield ) {
  ...
  for ( wh= weighth, w= weight, dh= datah, p= npix; p--; )
   if ( ( pix= *( wh++ ) ) < wthresh && pix > 0 && ( pix2= *( dh++ ) ) > -BIG && pix2 < satlevel )
    *( w++ )= 1 / sqrt( pix + ( pix2 > 0.0 ? ( gainflag ? pix2 * pix / backnoise2 : pix2 ) / gain
                                           : 0.0 ) );
   else {
    *( w++ )= 0.0;
    wbad++;
 ...
 if ( FLAG( obj2.chi2_psf ) ) {
  for ( j= 0; j < npsf; j++ ) {
   ...
   obj2->chi2_psf= flux[j] > 0 ? chi2 / ( ( npix - 3 * npsf ) * obj->sigbkg * obj->sigbkg ) : 999999;
  }
 }
 /* CHECK images */
 ...
 if ( prefs.pc_flag ) {
  ...
  if ( wfield ) {
   ...
   for ( w= weight, dh= datah, p= npix; p--; )
    *( w++ )= ( ( pix= *( dh++ ) ) > -BIG && pix < satlevel ) ? 1.0 / sqrt( backnoise2 + ( pix > 0.0 ? pix / gain : 0.0 ) )
                                                              : 0.0;
...
void double_psf_fit( psfstruct *psf, picstruct *field, picstruct *wfield,
 ...
 double *pmat,
     *pm,                                                           /* *pps,  *px, *py,*/
     ...
     gain, radmin2, radmax2, satlevel, chi2, pwthresh, pbacknoise2, /* mr, */
 ...
 if ( wfield ) {
  ...
  for ( pwh= pweighth, pw= pweight, pdh= pdatah, p= npix; p--; ) {
   if ( ( ppix= *( pwh++ ) ) < pwthresh && ppix > 0 && ( ppix2= *( pdh++ ) ) > -BIG && ppix2 < satlevel ) {
    *( pw++ )= 1 / sqrt( ppix + ( ppix2 > 0.0 ? ( gainflag ? ppix2 * ppix / pbacknoise2 : ppix2 ) / gain : 0.0 ) );
   } else {
    *( pw++ )= 0.0;
    wbad++;
   }
 ...
 for ( j= 1; j <= npsf; j++ ) {
  ...
  for ( k= 0; k < j; k++ ) {
   ...
   pvar= pcovmat;
   pfluxerr[k]= sqrt( *pvar ) > 0.0 && sqrt( *pvar ) < 99 ? sqrt( *pvar ) : 99;
  }
 }
 /* Compute chi2 if asked to 
  if (FLAG(obj2.chi2_psf))
 ...
 */
 ...
 if ( FLAG( obj2.chi2_psf ) ) {
  for ( j= 0; j < npsf; j++ ) {
   ...
   obj2->chi2_psf= pflux[j] > 0 ? chi2 / ( ( npix - 3 * npsf ) * obj->sigbkg * obj->sigbkg ) : 999999;
  }
 }
...
double *compute_gradient( float *weight, int width, int height,
                          float *masks, float *maskx, float *masky, double *m ) {
 int x, y;
 float *w, *ps, *px, *py;
 ...
 for ( y= 0; y < height; y++ )
  for ( x= 0; x < width; x++, ps++, w++ )
   *( m++ )= ( *( py++ )= ( y ? ( y >= ( height - 1 ) ? 0 : *( ps + width ) - *( ps - width ) ) : 0 ) ) * *w / 2;
 return m;
}
...
void compute_pos( int *pnpsf, int *pconvflag, int *pnpsfflag, double radmin2,
                  double radmax2, double r2, double *sol, double *flux, double *deltax, double *deltay, double *pdx, double *pdy ) {
 ...
 for ( j= 0; j < npsf; j++ ) {
  ...
  if ( flux[j] < 0.0 || ( deltax[j] * deltax[j] + deltay[j] * deltay[j] ) > radmax2 ) {
   npsfflag= 0;
   convflag= 0;
   npsf--;
   break;
