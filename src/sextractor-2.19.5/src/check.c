...
void blankcheck( checkstruct *check, PIXTYPE *mask, int w, int h,
 ...
 if ( xmin + w < 0 || xmin >= check->width || ymin + h < 0 || ymin >= check->height )
  return;
...
{
 if ( check->type == CHECK_APERTURES || check->type == CHECK_SUBPSFPROTOS || check->type == CHECK_SUBPCPROTOS || check->type == CHECK_PCOPROTOS || check->type == CHECK_SUBPROFILES || check->type == CHECK_SUBSPHEROIDS || check->type == CHECK_SUBDISKS || check->type == CHECK_OTHER ) {
