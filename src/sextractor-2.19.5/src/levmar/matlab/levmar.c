...
*/
 ...
 if ( !mxIsDouble( prhs[4] ) || mxIsComplex( prhs[4] ) || ( !( mxGetM( prhs[4] ) == 1 || mxGetN( prhs[4] ) == 1 ) && !( mxGetM( prhs[4] ) == 0 && mxGetN( prhs[4] ) == 0 ) ) )
 ...
 if ( nrhs >= 6 && mxIsChar( prhs[5] ) == 1 && mxGetM( prhs[5] ) == 1 ) {
  ...
  if ( !strncmp( minhowto, "unc", 3 ) )
   ...
   matlabFmtdErrMsgTxt( "levmar: unknown minimization type '%s'.", minhowto );
