 ...
 for ( l= 0; l < objlistin->nobj && out == RETURN_OK; l++ ) {
  ...
  for ( k= 1; k < xn; k++ ) {
   ...
   for ( i= 0; i < objlist[k - 1].nobj; i++ ) {
    if ( ( out= lutz( objlistin, l, &objlist[k - 1].obj[i], &debobjlist ) ) == RETURN_FATAL_ERROR )
     goto exit_parcelout;
   ...
   for ( i= 0; i < objlist[k].nobj; i++ ) {
    ...
    if ( m > 1 ) {
     for ( h= 0; ( j= (int)son[k + xn * ( i + NSONMAX * h )] ) != -1; h++ )
      if ( ok[k + 1 + xn * j] && obj[j].fdflux - obj[j].dthresh * obj[j].fdnpix > value0 ) {
       objlist[k + 1].obj[j].flag|= OBJ_MERGED /* Merge flag on */
                                    | ( ( OBJ_ISO_PB | OBJ_APERT_PB | OBJ_OVERFLOW ) & debobjlist2.obj[0].flag );
       if ( ( out= addobj( j, &objlist[k + 1], &debobjlist2 ) ) == RETURN_FATAL_ERROR )
        goto exit_parcelout;
      }
     ok[k + xn * i]= (short)0;
    }
