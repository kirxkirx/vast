...

{
 ...
 if ( prefs.prof_flag ) {
  ...
  modeltype= ( FLAG( obj2.prof_offset_flux ) ? MODEL_BACK : MODEL_NONE ) | ( FLAG( obj2.prof_dirac_flux ) ? MODEL_DIRAC : MODEL_NONE ) | ( FLAG( obj2.prof_spheroid_flux ) ? ( FLAG( obj2.prof_spheroid_sersicn ) ? MODEL_SERSIC : MODEL_DEVAUCOULEURS ) : MODEL_NONE ) | ( FLAG( obj2.prof_disk_flux ) ? MODEL_EXPONENTIAL : MODEL_NONE ) | ( FLAG( obj2.prof_bar_flux ) ? MODEL_BAR : MODEL_NONE ) | ( FLAG( obj2.prof_arms_flux ) ? MODEL_ARMS : MODEL_NONE );
 ...
 if ( !forcextflag ) {
  ...
  for ( ntab= 0; ntab < imacat->ntab; ntab++, imatab= imatab->nexttab ) {
   ...
   if ( ( imatab->naxis < 2 ) || !strncmp( imatab->xtension, "BINTABLE", 8 ) || !strncmp( imatab->xtension, "ASCTABLE", 8 ) )
    continue;
   next++;
  }
 }
 ...
 for ( ntab= 0; ntab < ntabmax; ntab++, imatab= imatab->nexttab ) {
  /*--  Check for the next valid image extension */
  if ( !forcextflag && ( ( imatab->naxis < 2 ) || !strncmp( imatab->xtension, "BINTABLE", 8 ) || !strncmp( imatab->xtension, "ASCTABLE", 8 ) ) )
   continue;
  ...
  if ( prefs.dimage_flag ) {
   ...
   /*---- Prepare interpolation */
   if ( ( prefs.dweight_flag || prefs.weight_flag ) && prefs.interp_type[0] == INTERP_ALL )
   ...
   if ( prefs.nweight_type > 1 ) {
    ...
    if ( prefs.weight_type[1] != WEIGHT_NONE ) {
     ...
     wfield= newweight( prefs.wimage_name[1], field, prefs.weight_type[1],
                        ( nima1 < 0 && prefs.image_name[1] ) ? ntab : ( nweight1 < 0 ? 1 : nweight1 ) );
  ...
  for ( i= 0; i < prefs.nimaflag; i++ ) {
   ...
   if ( ( pffield[i]->width != field->width ) || ( pffield[i]->height != field->height ) )
    error( EXIT_FAILURE,
  ...
  QPRINTF( OUTPUT, ( dfield || ( dwfield && dwfield->flags ^ INTERP_FIELD ) ) ? "(M)   "
                                                                              ...
                                                                              : "(M+D) "
                                                                                "Background: %-10g RMS: %-10g / Threshold: %-10g \n",
           field->backmean, field->backsig, ( field->flags & DETECT_FIELD ) ? field->dthresh : field->thresh );
  if ( dfield ) {
   QPRINTF( OUTPUT, "Detection image: " );
   makeback( dfield, dwfield ? dwfield : ( prefs.weight_type[0] == WEIGHT_NONE ? NULL : wfield ),
             prefs.wscale_flag[0] );
   QPRINTF( OUTPUT, "(D)   "
                    "Background: %-10g RMS: %-10g / Threshold: %-10g \n",
