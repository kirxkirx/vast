...
void endobject( picstruct *field, picstruct *dfield, picstruct *wfield,
 ...
 if ( prefs.assoc_flag && prefs.assocselec_type != ASSOCSELEC_ALL )
  selecflag= ( prefs.assocselec_type == ASSOCSELEC_MATCHED ) ? obj2->assoc_number : !obj2->assoc_number;
 else
  selecflag= 1;
 ...
 if ( selecflag ) {
  ...
  if ( FLAG( obj2.mx2w ) || FLAG( obj2.win_mx2w ) || FLAG( obj2.poserr_mx2w ) || FLAG( obj2.winposerr_mx2w ) || FLAG( obj2.poserrmx2w_psf ) || FLAG( obj2.poserrmx2w_prof ) || FLAG( obj2.prof_flagw ) || ( ( !prefs.pixel_scale ) && FLAG( obj2.area_flagw ) ) || ( ( !prefs.pixel_scale ) && FLAG( obj2.fwhmw_psf ) ) ) {
   rawpos[0]= obj2->posx;
   rawpos[1]= obj2->posy;
  ...
  if ( FLAG( obj2.npixw ) )
   obj2->npixw= obj->npix * ( prefs.pixel_scale ? field->pixscale / 3600.0 * field->pixscale / 3600.0 : obj2->pixscale2 );
  if ( FLAG( obj2.fdnpixw ) )
   obj2->fdnpixw= obj->fdnpix * ( prefs.pixel_scale ? field->pixscale / 3600.0 * field->pixscale / 3600.0 : obj2->pixscale2 );

  if ( FLAG( obj2.fwhmw ) )
   obj2->fwhmw= obj->fwhm * ( prefs.pixel_scale ? field->pixscale / 3600.0 : sqrt( obj2->pixscale2 ) );
  ...
  if ( FLAG( obj2.fwhm_psf ) || ( FLAG( obj2.sprob ) && prefs.seeing_fwhm == 0.0 ) ) {
   ...
   if ( FLAG( obj2.fwhmw_psf ) )
    obj2->fwhmw_psf= obj2->fwhm_psf * ( prefs.pixel_scale ? field->pixscale / 3600.0 : sqrt( obj2->pixscale2 ) );
  }
  ...
  if ( FLAG( obj2.sprob ) ) {
   ...
   input[++j]= field->thresh > 0.0 ? log10( obj->peak > 0.0 ? obj->peak / field->thresh : 0.1 )
                                   : -1.0;
   for ( i= 1; i < NISO; i++ )
  ...
  for ( j= 0; j < nsub; j++ ) {
   if ( prefs.psffit_flag ) {
    ...
    if ( FLAG( obj2.mag_psf ) )
     obj2->mag_psf= thepsfit->flux[j] > 0.0 ? prefs.mag_zeropoint - 2.5 * log10( thepsfit->flux[j] ) : 99.0;
    ...
    if ( FLAG( obj2.magerr_psf ) )
     obj2->magerr_psf=
         ( thepsfit->flux[j] > 0.0 && thepsfit->fluxerr[j] > 0.0 ) ? /*?*/
             1.086 * thepsfit->fluxerr[j] / thepsfit->flux[j]
                                                                   : 99.0;
    if ( j )
