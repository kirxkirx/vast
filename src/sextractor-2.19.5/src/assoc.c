...

{
 ...
 ispoon= ncol= ndata= nlist= size= spoonsize= xindex= yindex= mindex= 0;
 ...
 for ( i= 0; fgets( str, MAXCHARL, file ); ) {
  ...
  if ( !i ) {
   ...
   if ( prefs.nassoc_param > 2 ) {
    ...
    mindex= -1;
    if ( prefs.assoc_type == ASSOC_MEAN || prefs.assoc_type == ASSOC_MAGMEAN || prefs.assoc_type == ASSOC_MIN || prefs.assoc_type == ASSOC_MAX ) {
...

{
 ...
 if ( !( assoc= field->assoc= load_assoc( prefs.assoc_name,
                                          prefs.assoccoord_type == ASSOCCOORD_WORLD ? field->wcs : NULL ) ) )
