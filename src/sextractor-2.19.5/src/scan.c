...

{
 ...
 scan= dscan= cdscan= cdwscan= cdwscann= cdwscanp= dwscan= dwscann= dwscanp= wscan= wscann= wscanp= NULL;
 ...
 for ( yl= 0; yl <= h; ) {
  ...
  if ( yl == h ) {
   ...
   if ( nffield )
    for ( i= 0; i < nffield; i++ ) {
     ffield= pffield[i];
     pfscan[i]= ( ffield->stripy == ffield->stripysclim ) ? (FLAGTYPE *)loadstrip( ffield, (picstruct *)NULL )
                                                          : &ffield->fstrip[ffield->stripy * ffield->width];
    }
   if ( wfield ) {
    /*------ Copy the previous weight line to track bad pixel limits */
    wscan= ( wfield->stripy == wfield->stripysclim ) ? (PIXTYPE *)loadstrip( wfield, (picstruct *)NULL )
   ...
   scan= ( field->stripy == field->stripysclim ) ? (PIXTYPE *)loadstrip( field, wfield )
                                                 : &field->strip[field->stripy * field->width];
   if ( dwfield ) {
    dwscan= ( dwfield->stripy == dwfield->stripysclim ) ? (PIXTYPE *)loadstrip( dwfield,
                                                                                dfield ? (picstruct *)NULL : dwfield )
                                                        : &dwfield->strip[dwfield->stripy * dwfield->width];
    if ( PLISTEXIST( wflag ) ) {
     if ( yl > 0 )
      dwscanp= &dwfield->strip[( ( yl - 1 ) % dwfield->stripheight ) * dwfield->width];
     if ( yl < h - 1 )
      dwscann= &dwfield->strip[( ( yl + 1 ) % dwfield->stripheight ) * dwfield->width];
    }
   } else {
    dwscan= wscan;
    if ( PLISTEXIST( wflag ) ) {
   ...
   if ( dfield )
    dscan= ( dfield->stripy == dfield->stripysclim ) ? (PIXTYPE *)loadstrip( dfield, dwfield )
  ...
  if ( cfield->stripy == cfield->stripysclim ) {
   ...
   for ( i= cleanobjlist->nobj; i--; cleanobj-- ) {
    if ( cleanobj->ycmin <= cfield->ymin ) {
     ...
     if ( ( prefs.prof_flag && !( thecat.ntotal % 10 ) && thecat.ntotal != ontotal ) || !( thecat.ntotal % 400 ) )
...

{
 ...
 if ( analyse_type & ANALYSE_FULL ) {
  ...
  if ( t1t2 > 0.0 && !prefs.dweight_flag ) {
   obj->abcor= ( darea < 0.0 ? darea : -1.0 ) / ( 2 * PI * log( t1t2 < 1.0 ? t1t2 : 0.99 ) * obj->a * obj->b );
   if ( obj->abcor > 1.0 )
    obj->abcor= 1.0;
  } else
   obj->abcor= 1.0;
