...
#define FITSREADS( buf, k, str, def )                             \
 ...
 /* Test if the WCS is recognized and a celestial pair is found */
 if ( !wcsset( wcs->naxis, (const char( * )[9])wcs->ctype, wcs->wcsprm ) && wcs->wcsprm->flag < 999 ) {
  char *pstr;
  double date;
  int biss, dpar[3];
  ...
  if ( date > 0.0 )
   ...
   if ( *str ) {
    ...
    biss= ( dpar[0] % 4 ) ? 0 : 1;
    /*------ Convert date to MJD */
    date= -678956 + ( 365 * dpar[0] + dpar[0] / 4 ) - biss + ( ( dpar[1] > 2 ? ( (int)( ( dpar[1] + 1 ) * 30.6 ) - 63 + biss ) : ( ( dpar[1] - 1 ) * ( 63 + biss ) ) / 2 ) + dpar[2] );
    wcs->obsdate= 2000.0 - ( MJD2000 - date ) / 365.25;
   } else
...

{
 ...
 if ( !strcmp( wcs->wcsprm->pcode, "TNX" ) )
  tnxflag= 1;
 else if ( ( !strcmp( wcs->wcsprm->pcode, "TAN" ) || !strcmp( wcs->wcsprm->pcode, "TPV" ) ) && ( wcs->projp[1 + lng * 100] || wcs->projp[1 + lat * 100] ) )
  tnxflag= 0;
 else
  return;
 ...
 for ( j= WCS_NGRIDPOINTS; j--; raw[lat]+= latstep ) {
   ...
   *( latpost++ )= pixin[lat];
   if ( tnxflag ) {
    *( outpost++ )= pixin[lng] + raw_to_tnxaxis( wcs->tnx_lngcor, pixin[lng], pixin[lat] );
    *( outpost++ )= pixin[lat] + raw_to_tnxaxis( wcs->tnx_latcor, pixin[lng], pixin[lat] );
   } else {
...

{
 ...
 swapflag= ( ( ( wcsin->lng != wcsout->lng ) || ( wcsin->lat != wcsout->lat ) ) && ( wcsin->lng != wcsin->lat ) && ( wcsout->lng != wcsout->lat ) );
...

{
 ...
 if ( wcs->lng != wcs->lat ) {
  ...
  if ( dpos2 > 180.0 )
   ...
   dpos2+= 360.0;
  return fabs( ( dpos1 * ( wcspos2[lat] - wcspos[lat] ) - ( wcspos1[lat] - wcspos[lat] ) * dpos2 ) * cos( wcspos[lat] * DEG ) );
 } else
  return fabs( ( dpos1 * ( wcspos2[lat] - wcspos[lat] ) - ( wcspos1[lat] - wcspos[lat] ) * dpos2 ) );
}
...
double wcs_jacobian( wcsstruct *wcs, double *pixpos, double *jacob ) {
 ...
 return fabs( jacob[lng + naxis * lng] * jacob[lat + naxis * lat] - jacob[lat + naxis * lng] * jacob[lng + naxis * lat] );
}
...

{
 ...
 angle= ( dalpha != 0.0 && ( crval[lat] - wcs->crval[lat] ) * DEG != 0.0 ) ? 180.0 - ( atan2( sin( dalpha ),
                                                                                              cos( crval[lat] * DEG ) * tan( wcs->crval[lat] * DEG ) - sin( crval[lat] * DEG ) * cos( dalpha ) ) +
                                                                                       atan2( sin( dalpha ),
                                                                                              cos( wcs->crval[lat] * DEG ) * tan( crval[lat] * DEG ) - sin( wcs->crval[lat] * DEG ) * cos( dalpha ) ) ) /
                                                                                         DEG
                                                                           : 0.0;
...

{
 ...
 cddcadz= -( sdin= sin( deltain ) ) * ( st= sin( theta ) ) + cdin * ( ct= cos( theta ) ) * ( caindz= cos( alphain + dzeta ) );
