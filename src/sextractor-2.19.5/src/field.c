...

{
 ...
 nok= 0;
 tab= cat->tab;
 if ( tab->naxis >= 2 && strncmp( tab->xtension, "BINTABLE", 8 ) && strncmp( tab->xtension, "ASCTABLE", 8 ) )
  nok++;
 ext2= ext;
 ...
 QPRINTF( OUTPUT, "----- %s %s%s\n",
          flags & FLAG_FIELD ? "Flagging  from:" : ( flags & ( RMS_FIELD | VAR_FIELD | WEIGHT_FIELD ) ? "Weighting from:" : ( flags & MEASURE_FIELD ? "Measuring from:" : "Detecting from:" ) ),
          field->rfilename,
          cat->ntab > 1 ? gstr : "" );
 QPRINTF( OUTPUT, "      \"%.20s\" / %s / %dx%d / %d bits %s\n",
          ...
          field->bitpix > 0 ? ( field->tab->compress_type != COMPRESS_NONE ? "(compressed)" : "(integers)" )
                            : "(floats)" );
 ...
 if ( flags & ( DETECT_FIELD | MEASURE_FIELD | WEIGHT_FIELD | VAR_FIELD | RMS_FIELD ) ) {
  ...
  if ( ( ( flags & DETECT_FIELD ) && prefs.back_type[0] == BACK_ABSOLUTE ) || ( ( flags & MEASURE_FIELD ) && prefs.back_type[1] == BACK_ABSOLUTE ) )
