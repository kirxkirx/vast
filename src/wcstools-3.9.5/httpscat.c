...
main() {
 ...
 if ( !( fp= scathttp( "http://cfa-www/catalog/scat", "ua2", 10.0, 10.0, "J2000", 360.0, 20.0, "" ) ) ) {
...
File scathttp( url, catalog, ra, dec, system, radius, mag, sort ) char *url;
char *catalog;
double ra;
double dec;
char *system;
...
char *sort;
{
 ...
 sprintf( URL, "/%s?catalog=%s&ra=%f&dec=%f&system=%s&radius=%f&&mag=%f&sort=%s", page, catalog, ra, dec, system, radius, mag, sort );
...
File SokOpen( name, port, mode ) char *name; /* "host:port" socket to open */
int port;
int mode; /* mode of socket to open */
{
 ...
 if ( type == 1 || ( mode & XFCREAT && mode & XFREAD && !( mode & XFWRITE ) ) ) {
  if ( ( ( xfd= socket( AF_INET, SOCK_STREAM, 0 ) ) < 0 ) || setsockopt( xfd, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse_addr, sizeof( reuse_addr ) ) < 0 || ( bind( xfd, (struct sockaddr *)&adrinet, sizeof( adrinet ) ) != 0 ) || listen( xfd, 5 ) ) {
   close( xfd );
   return NULL;
  }
 } else {
  if ( ( ( xfd= socket( AF_INET, SOCK_STREAM, 0 ) ) < 0 ) || ( connect( xfd, (struct sockaddr *)&adrinet, sizeof( adrinet ) ) != 0 ) ) {
   close( xfd );
   return NULL;
  }
 }
...
static int FileINetParse( file, port, adrinet ) char *file; /* host/socket pair to parse? */
int port;
...
static char *iaddrstr( addr, addrlen ) struct sockaddr_in *addr; /* socket info structure to fill? */
int addrlen;
{
 struct hostent *hp= NULL;
 char *name;

 if ( !( hp= gethostbyaddr( (char *)&addr->sin_addr, sizeof( addr->sin_addr ), AF_INET ) ) ) {
  unsigned char *a= (unsigned char *)&addr->sin_addr;

  Malloc( name, 32 );
  sprintf( name, "%d.%d.%d.%d:%d", a[0], a[1], a[2], a[3], ntohs( addr->sin_port ) );
 } else {
