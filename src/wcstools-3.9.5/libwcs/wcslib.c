...

{
 ...
 for ( j= 0; j < naxis; j++ ) {
  ...
  for ( k= 0; k < npcode; k++ ) {
   if ( strncmp( &ctype[j][5], pcodes[k], 3 ) == 0 )
    break;
  }
  ...
  if ( k == npcode ) {
   ...
   for ( k= 0; k < nalias; k++ ) {
    if ( strncmp( &ctype[j][5], aliases[k], 3 ) == 0 )
     break;
   }
 ...
 *theta= -90.0;
  ...
  *theta= -*theta;
  ...
  if ( mixcel == 1 ) {
   if ( fabs( fmod( world[wcs->lng] - lng, 360.0 ) ) > tol )
    continue;
   if ( lat < span[0] )
    continue;
   if ( lat > span[1] )
