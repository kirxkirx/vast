...
char *gettaberr() { return ( taberr ); }

int gettabndec() { return ( nndec ); }

static char *kwo= NULL; /* Keyword returned by tabread(), tabrnum() */
void settabkey( keyword0 ) char *keyword0;
{
 kwo= keyword0;
 return;
}
...
int nbbuff; /* Number of bytes in buffer; 0=read whole file */
{
 ...
 for ( i= 0; i < startab->ncols; i++ ) {
  ...
  if ( strcsrch( keyword, "mag" ) && !strcsrch( keyword, "err" ) && !strcsrch( keyword, "code" ) ) {
...
void
    tabcatclose( sc ) struct StarCat *sc;
{
 tabclose( sc->startab );
 free( sc );
 return;
...
{
 if ( tabtable != NULL ) {
  if ( tabtable->filename != NULL )
   free( tabtable->filename );
  if ( tabtable->tabname != NULL )
   free( tabtable->tabname );
  if ( tabtable->tabbuff != NULL )
   free( tabtable->tabbuff );
  if ( tabtable->colname != NULL )
   free( tabtable->colname );
  if ( tabtable->lcol != NULL )
   free( tabtable->lcol );
  if ( tabtable->lcfld != NULL )
   free( tabtable->lcfld );
  if ( tabtable->tcat != NULL )
   fclose( tabtable->tcat );
  free( tabtable );
 }
 return;
}
