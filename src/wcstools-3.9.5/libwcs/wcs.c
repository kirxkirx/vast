...
void
    wcsfree( wcs ) struct WorldCoor *wcs; /* WCS structure */
{
 if ( nowcs( wcs ) ) {
...
double crota;                  /* Rotation counterclockwise in degrees */
{
 ...
 if ( wcs->coorflip ) {
  if ( wcs->cdelt[0] < 0 && wcs->cdelt[1] > 0 ) {
   ...
   if ( wcs->imrot < -180.0 )
    wcs->imrot= wcs->imrot + 360.0;
   wcs->pa_north= wcs->rot;
   wcs->pa_east= wcs->rot - 90.0;
   if ( wcs->pa_east < -180.0 )
...
double
    wcsdist( x1, y1, x2, y2 )
        ...
        double x1,
    y1;        /* (RA,Dec) or (Long,Lat) in degrees */
...
char *
    getwcsout( wcs ) struct WorldCoor *wcs; /* World coordinate system structure */
{
 if ( nowcs( wcs ) )
  return ( NULL );
 else
...
char *
    getwcsin( wcs ) struct WorldCoor *wcs; /* World coordinate system structure */
{
 if ( nowcs( wcs ) )
  return ( NULL );
 else
...
int
    setwcsdeg( wcs, new ) struct WorldCoor *wcs; /* World coordinate system structure */
...
int
    wcsndec( wcs, ndec ) struct WorldCoor *wcs; /* World coordinate system structure */
...
char *
    getradecsys( wcs ) struct WorldCoor *wcs; /* World coordinate system structure */
{
 if ( nowcs( wcs ) )
  return ( NULL );
 else
...
void
    setwcslin( wcs, mode ) struct WorldCoor *wcs; /* World coordinate system structure */
...
void
    setwcsfile( filename ) char *filename; /* FITS or IRAF file with WCS */
{
...
/* Set error message */
void
    setwcserr( errmsg ) char *errmsg; /* Error mesage  < 80 char */
{
 strcpy( wcserrmsg, errmsg );
 return;
}
...
void
    setwcscom( wcs ) struct WorldCoor *wcs; /* WCS parameter structure */
{
 char envar[16];
 int i;
 char *str;
...
void
    freewcscom( wcs ) struct WorldCoor *wcs; /* WCS parameter structure */
{
 int i;
 for ( i= 0; i < 10; i++ ) {
  if ( wcscom0[i] != NULL ) {
