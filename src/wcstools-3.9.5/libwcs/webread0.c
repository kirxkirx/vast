...
static FILE *
    SokOpen( name, port, mode ) char *name; /* "host:port" socket to open */
int port;
...
static int
    FileINetParse( file, port, adrinet ) char *file; /* host/socket pair to parse? */
int port;
...
char *
    space2tab( tabbuff ) char *tabbuff; /* Tab table filled with spaces */
{
