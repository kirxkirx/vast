...
void setimcat( cat ) char *cat;
{
 strcpy( imcatname, cat );
 return;
}

/* Get input catalog for image stars */
char *getimcat() { return ( imcatname ); }

static int nspix= NSTATPIX; /* Stats are computed for +- this many pixels */
void setnspix( nsp ) int nsp;
{
 nspix= nsp;
 return;
}
...
void setburnedout( bmax ) double bmax;
{
 burnedout= bmax;
 return;
}
...
void setstarsig( sig ) double sig;
{
 starsig= sig;
 return;
}
...
static int mirror= 0;
void setmirror( mirror1 ) int mirror1;
{
 mirror= mirror1;
 return;
}
...
static int rotate= 0;
void setrotate( rotate1 ) int rotate1;
{
 rotate= rotate1;
 return;
}
...
static void
    mean1d( sv1, sv2, mean, sigma )
        ...
        double *sv1,
    *sv2;      /* starting and ending pixels for statistics */
...
void
    setparm( parstring ) char *parstring;
{
 char *parname;
 char *parvalue;
