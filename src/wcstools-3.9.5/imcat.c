...
static char *RevMsg= "IMCAT WCSTools 3.9.5, 30 March 2017, Jessica Mink (jmink@cfa.harvard.edu)";
 ...
 for ( icat= 0; icat < ncat; icat++ ) {
  ...
  if ( region_radius[0] ) {
   ...
   if ( region_radius1[icat] != region_radius[icat] ) {
    ...
    for ( i= 0; i < nbg; i++ ) {
     if ( gm[magsort][i] > max_mag )
      max_mag= gm[magsort][i];
     if ( gm[magsort][i] < min_mag )
      min_mag= gm[magsort][i];
    }
    if ( max_mag == min_mag ) {
