...
should you start this script from the VaST home directory?" >> /dev/stderr
 exit 1
fi

### Ask the user
while true ;do
 echo "Do you want to download the full UCAC5 catalog (5.3G)? (yes/no)" >> /dev/stder
 read USERS_ANSWER
...
if [ "$USERS_ANSWER" = "no" ];then
 ...
 echo "Downloading the catalog" >> /dev/stder
fi
...
if [ $? -eq 0 ];then
 echo "Download complete" >> /dev/stder
else
 echo "ERROR running wget" >> /dev/stder
fi
