...
"$CC" $(cat optflags_for_scripts.tmp) -o lc setenv_local_pgplot.o lc.o variability_indexes.o quickselect.o get_path_to_vast.o wpolyfit.o -lm $CFITSIO_LIB $GSL_LIB $PGPLOT_LIBS  -Wall
# Older GCC versions complain about isnormal() unless -std=c99 is given explicitly
...
"$CC" $(cat optflags_for_scripts.tmp) -o pgfv pgfv.o setenv_local_pgplot.o photocurve.o vast_utils.o gettime.o kourovka_sbg_date.o autodetect_aperture.o guess_saturation_limit.o get_number_of_cpu_cores.o exclude_region.o replace_file_with_symlink_if_filename_contains_white_spaces.o variability_indexes.o quickselect.o get_path_to_vast.o is_point_close_or_off_the_frame_edge.o $CFITSIO_LIB -lm $GSL_LIB -I$GSL_INCLUDE $PGPLOT_LIBS
