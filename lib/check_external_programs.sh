...
for TESTED_PROGRAM in $CC $FC $CXX awk sed bc wc cat tac cut sort uniq touch head tail grep basename ping curl wget `lib/find_timeout_command.sh` find readlink file $MD5COMMAND df du gzip gunzip $C_HEADER_FILES $X11_DEVELOPEMENT_PACKAGE ;do
