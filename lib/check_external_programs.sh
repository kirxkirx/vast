...
for TESTED_PROGRAM in $CC $FC $CXX awk sed bc wc cat cut sort uniq touch head tail grep basename ping curl $(lib/find_timeout_command.sh) find readlink file $MD5COMMAND df du gzip gunzip mktemp tee tr $C_HEADER_FILES $X11_DEVELOPEMENT_PACKAGE ;do
